---
title: "Створення модуля CRUD"
description: "Повний підручник зі створення модуля CRUD з операціями з базою даних, формами та інтерфейсом адміністратора"
---
# Підручник зі створення модуля CRUD

Цей підручник допоможе вам створити повний модуль CRUD (Створення, читання, оновлення, видалення) для XOOPS. Ми створимо модуль «Нотатки», який дозволить користувачам керувати особистими нотатками.

## Передумови

- Виконано підручник Hello-World-Module
- Розуміння концепцій ООП PHP
- Базові знання SQL

## Огляд модуля

**Особливості модуля Notes:**
- Створення, перегляд, редагування та видалення нотаток
— Інтерфейс керування адміністратором
- Примітки для користувача
- Категорійна організація
- Функція пошуку

## Крок 1: Структура каталогу

Створіть таку структуру в `/modules/notes/`:
```
/modules/notes/
    /admin/
        admin_header.php
...
```
## Крок 4: Класи сутностей

### Примітка

Створити `class/Note.php`:
```php
<?php
/**
 * Note Entity Class
...
Створити `class/NoteHandler.php`:
```php
<?php
/**
 * Note Handler Class
...
Створити `class/CategoryHandler.php`:
```php
<?php
/**
...
```
## Крок 5: Загальний файл включення

Створити `include/common.php`:
```php
<?php
/**
 * Common Include File
...
require XOOPS_ROOT_PATH . '/footer.php';
```
### Сторінка редагування (Create/Update)
...
Створити `templates/notes_index.tpl`:
```smarty
...
Створити `templates/notes_view.tpl`:
```smarty
<{* Note View Template *}>
...
Створити `templates/notes_edit.tpl`:
```smarty
<{* Note Edit Template *}>
...
```
## Крок 9: Інтерфейс адміністратора
...
```
### Керування нотатками адміністратора

Створити `admin/notes.php`:
```php
<?php
/**
...
Щиро вітаю! Ви створили повний модуль CRUD. Основні поняття, що розглядаються:

1. **Дизайн бази даних** - Таблиці зі зв'язками
2. **Entity Classes** - XoopsObject із типізованими властивостями
3. **Класи обробників** - XoopsPersistableObjectHandler із спеціальними методами
4. **Frontend Pages** - Функція списків, перегляду та редагування
5. **Обробка форм** - захист і перевірка CSRF
6. **Інтерфейс адміністратора** - Екрани керування
7. **Шаблони** - Smarty шаблони з логікою

## Наступні кроки

- Додайте додаткові функції (коментарі, оцінки, обмін)
- Впровадження ../Patterns/Repository-Pattern для чистішого доступу до даних
— Застосуйте ../Patterns/MVC-Pattern для кращої організації коду
— Додайте ../Best-Practices/Testing за допомогою PHPUnit

Дивіться також: ../Module-Development | ../Patterns/MVC-Pattern | ../Patterns/Repository-Pattern