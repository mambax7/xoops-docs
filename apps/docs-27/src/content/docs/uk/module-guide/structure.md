...
Добре організована структура модуля є фундаментальною для підтримки розробки XOOPS. Цей посібник охоплює як застарілі, так і сучасні (PSR-4) макети модулів.

## Стандартна схема модуля
...
### xoops_version.php (застарілий маніфест)
```php
<?php
$modversion = [
    'name'           => 'My Module',
...
```
### module.json (маніфест XOOPS 4.0)
```json
{
    "name": "My Module",
    "slug": "mymodule",
...
|-----------|---------|
| `admin/` | Інтерфейс адміністрування |
| `assets/` | CSS, JavaScript, зображення |
| `blocks/` | Функції рендеринга блоків |
| `class/` | PHP класи (спадщина) |
| `config/` | Конфігураційні файли (сучасні) |
| `include/` | Спільні включені файли |
| `language/` | Файли перекладу |
| `migrations/` | Міграції бази даних |
| `sql/` | Початкова схема бази даних |
| `src/` | Вихідний код PSR-4 |
| `templates/` | Шаблони Smarty |
| `tests/` | Тестові файли |

## Найкращі практики

1. **Окремі проблеми** - Не використовуйте бізнес-логіку в шаблонах
2. **Використовуйте простори імен** – організуйте код із належним простором імен
3. **Дотримуйтеся PSR-4** - використовуйте стандартні правила автозавантаження
4. **Externalize Config** - Зберігайте конфігурацію окремо від коду
5. **Структура документа** - Включіть README, що пояснює організацію

## Пов'язана документація

- Module-Development - повний посібник із розробки
- Best-Practices/Code-Organization - шаблони організації коду
- Маніфест модуля - Конфігурація маніфесту
- Database/Database-Schema - Дизайн бази даних