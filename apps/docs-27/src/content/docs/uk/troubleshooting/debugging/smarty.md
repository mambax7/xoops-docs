---
title: "Налагодження шаблону Smarty"
description: "Розширені методи налагодження шаблонів і усунення несправностей для XOOPS"
---
> Розширені методи налагодження шаблонів Smarty у темах і модулях XOOPS.
...
## Увімкнути режим налагодження Smarty

### Спосіб 1: Панель адміністратора

XOOPS Адміністратор > Налаштування > Продуктивність:
- Увімкнути «Вивід налагодження»
- Встановіть «Рівень налагодження» на 2

---

### Спосіб 2: Конфігурація коду
```php
<?php
...
### Спосіб 3: спливаюче вікно налагодження у браузері
```smarty
{* Add to template to enable debug in footer *}
{debug}
```
Це показує спливаюче вікно з усіма призначеними змінними.

---

## Загальні методи налагодження Smarty
...
## Перегляд скомпільованих шаблонів

Smarty компілює шаблони до PHP для продуктивності. Налагодження, переглядаючи скомпільований код:
```bash
...
## Налагодити конкретні проблеми

### Проблема 1: Змінна відображається як порожня
```php
<?php
$tpl = new XoopsTpl();
...
### Проблема 2: ключ масиву не знайдено
```smarty
...
### Випуск 3: Plugin/Modifier не знайдено
```php
<?php
...
### Проблема 4: Відображення вкладеного масиву
```smarty
{* Debug nested arrays *}
...
### Покроковий процес налагодження
```mermaid
graph TD
...
## Допоміжні функції налагодження
```php
<?php
