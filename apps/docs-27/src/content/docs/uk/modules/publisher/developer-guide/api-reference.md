---
title: "Видавець - API Довідка"
description: "Повний довідник API для модуля Publisher із класами, методами та прикладами коду"
---
# Видавець API Посилання

> Повний довідник для класів модулів Publisher, методів, функцій і кінцевих точок API.
...
Клас `Item` представляє один article/item у Publisher.

**Простір імен:** `XoopsModules\Publisher\`
...
`ItemHandler` керує операціями CRUD для статей.

**Файл:** `modules/publisher/class/ItemHandler.php`
...
Клас `Category` представляє категорію або розділ.

**Файл:** `modules/publisher/class/Category.php`
...
`CategoryHandler` керує операціями категорії CRUD.

**Файл:** `modules/publisher/class/CategoryHandler.php`
...
Клас Helper надає допоміжні функції:

**Файл:** `modules/publisher/class/Helper.php`
```php
// Get helper instance
...
### Інтерфейсні функції JavaScript

Publisher включає JavaScript API для взаємодії з інтерфейсом:
```javascript
...
Publisher надає кінцеві точки AJAX для зовнішньої взаємодії:
```javascript
...
## REST API (якщо ввімкнено)

### API Кінцеві точки

Якщо видавець надає REST API:
```
...
?>
```
### Отримайте статті за категоріями
```php
<?php
...
- Аналіз модуля видавця
- Шаблони та блоки в Publisher
- Створення статті
