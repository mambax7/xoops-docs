---
title: "Видавець - Хуки та події"
description: "Посібник із розширення Publisher за допомогою хуків, подій і системи плагінів"
---
# Хуки та події видавців

> Повний посібник із розширення функціональності Publisher за допомогою подій, хуків і плагінів.
...
Події дозволяють іншим модулям реагувати на дії видавця:
```
...
Звільняється, коли створюється нова стаття.
```php
...
Звільняється, коли стаття оновлюється.
```php
...
Звільняється при видаленні статті.
```php
...
Викликається, коли статус статті змінюється на опубліковано.
```php
...
Звільнено, коли статтю, що очікує на розгляд, схвалено.
```php
...
Звільняється, коли стаття відхилена.
```php
...
Запускається під час створення категорії.
```php
...
Запускається під час оновлення категорії.
```php
...
Запускається, коли категорію видалено.
```php
...
Звільняється після публікації коментаря.
```php
...
Звільняється після схвалення коментаря.
```php
...
Звільняється, коли коментар видалено.
```php
...
### Приклад 1: Надіслати електронний лист про створення статті
```php
<?php
// Listen for article creation
xoops_events()->attach(
...
?>
```
### Приклад 2: Оновлення індексу пошуку
```php
<?php
...
?>
```
### Приклад 3: Автоматична публікація в соціальних мережах
```php
<?php
...
?>
```
### Приклад 4: Синхронізація із зовнішньою системою
```php
<?php
...
Хуки дозволяють змінювати поведінку видавця:

#### publisher.view.article.start

Викликається перед відтворенням статті.
```php
xoops_events()->attach(
    'publisher.view.article.start',
...
Викликається після відтворення статті.
```php
xoops_events()->attach(
    'publisher.view.article.end',
    'append_to_article'
...
Викликається під час перевірки дозволів.
```php
xoops_events()->attach(
    'publisher.permission.check',
...
Плагіни розширюють функціональність Publisher:
...
Фільтри змінюють дані обробки before/after:
```php
<?php
...
### Приклад плагіна: пов’язані статті
```php
<?php
...
- [Видавець GitHub](https://github.com/XoopsModules25x/publisher)
- [XOOPS Система подій](../../03-Module-Development/Module-Development.md)
- [Розробка плагіна](../../03-Module-Development/Module-Development.md)
