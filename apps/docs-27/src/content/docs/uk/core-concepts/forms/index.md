---
title: "Форми XOOPS"
description: "Повний посібник із системи генерації форм XOOPS, включаючи всі елементи форми, перевірку та рендеринг"
---
# 📝 Система форм XOOPS

> Комплексне створення форм, перевірка та рендеринг для модулів XOOPS.

---

## Огляд

Система форм XOOPS забезпечує потужний об’єктно-орієнтований підхід до створення форм HTML. Він забезпечує створення форм, перевірку, захист CSRF і гнучке відтворення з підтримкою різних фреймворків CSS.

---

## 🚀 Швидкий старт

### Створення базової форми
```php
<?php
use XoopsFormButton;
use XoopsFormHidden;
...
### XoopsForm (базовий клас)

Абстрактний базовий клас для всіх форм.
```php
...
Найпоширеніший клас форми, візуалізація зі стилем теми.
```php
...
```
### Панель елементів (група елементів)
```php
...
Завжди включайте прихований маркер:
```php
$form->addElement(new XoopsFormHiddenToken());
...
```
### Перевірте маркер під час надсилання
```php
if (!$GLOBALS['xoopsSecurity']->check()) {
...
- Перевірка форми
- Настроювані засоби візуалізації форм
- CSRF Захист
- Дезінфекція входу
