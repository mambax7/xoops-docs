---
title: "Система дозволів"
description: "Повний посібник із системи дозволів XOOPS, типів, перевірки, ієрархії та контролю доступу"
---
# Система дозволів у XOOPS

Система дозволів XOOPS — це детальна структура контролю доступу, яка керує тим, хто може виконувати дії на яких ресурсах. Цей документ охоплює типи дозволів, механізми перевірки, ієрархію та приклади реалізації.

## Типи дозволів

### Дозволи на рівні модуля

Дозволи на рівні модуля контролюють доступ до цілих модулів або функцій модуля.

**Поширені назви дозволів:**
- `module_view` - Перегляд вмісту модуля
- `module_read` - Читання ресурсів модуля
- `module_submit` - Надіслати вміст до модуля
- `module_edit` - Редагувати вміст модуля
- `module_admin` - Модуль адміністрування
```php
<?php
/**
...
```
### Дозволи на рівні елементів

Дозволи на рівні елементів контролюють доступ до певних ресурсів у межах модуля.

**Приклади:**
- Ідентифікатор статті: чи можна групувати view/edit певну статтю?
- Ідентифікатор категорії: Чи може група отримати доступ до категорії?
- Ідентифікатор сторінки: чи можна групувати view/modify певну сторінку?
```php
<?php
/**
...
Дозволи блоків контролюють видимість і взаємодію з блоками, що відображаються на сторінках.
```php
<?php
/**
...
Дозволи контролюють керування та адміністрування групи.
```php
<?php
/**
...
graph TB
    ...
    DELETE --> GRANT
```
### Ланцюжок успадкування дозволів
```mermaid
sequenceDiagram
...
Клас `XoopsGroupPermHandler` надає методи для перевірки та керування дозволами.
```php
<?php
/**
...
```
## Реалізація перевірки дозволів

### Перевірка дозволу для одного користувача
```php
<?php
/**
...
```
### Проміжне програмне забезпечення дозволів
```php
<?php
/**
...
```
### Перевірка дозволів панелі адміністратора
```php
<?php
/**
...
### Оптимізована перевірка дозволів
```php
<?php
/**
...
### Правила призначення дозволів

1. **Принцип найменших привілеїв**: призначайте лише необхідні дозволи
2. **Доступ на основі ролей**: використовуйте групи для дозволів на основі ролей
3. **Регулярні аудити**: періодично переглядайте дозволи
4. **Розподіл обов’язків**: відокремте права адміністратора від дозволів користувача
5. **Явна заборона**: заборона за замовчуванням, підхід явного дозволу
...
#дозволи #контроль доступу #безпека #авторизація #acl #перевірка дозволів