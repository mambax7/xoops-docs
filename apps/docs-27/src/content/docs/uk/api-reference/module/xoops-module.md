---
title: "Довідка XoopsModule API"
description: "Повний довідник API для XoopsModule і системних класів модулів"
---
> Повна документація API для модульної системи XOOPS.
...
|----------|------|-------------|
| `mid` | int | ID модуля |
| `name` | рядок | Відображуване ім'я |
| `version` | рядок | Номер версії |
| `dirname` | рядок | Назва каталогу |
| `isactive` | int | Активний статус (0/1) |
| `hasmain` | int | Має основну площу |
| `hasadmin` | int | Має адмінзону |
| `hassearch` | int | Має функцію пошуку |
| `hasconfig` | int | Має конфігурацію |
| `hascomments` | int | Є коментарі |
| `hasnotification` | int | Має сповіщення |
...
## Послідовність встановлення модуля
```mermaid
sequenceDiagram
    participant Admin
    participant System
...
#xoops #api #module #xoopsmodule #посилання