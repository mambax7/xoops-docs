---
title: "Smarty Шаблон API Посилання"
description: "Повний довідник API для шаблонів Smarty у XOOPS"
---
> Повна документація API для створення шаблонів Smarty у XOOPS.

---

## Архітектура механізму шаблонів
```mermaid
graph TB
...
|--------|------------|-------------|
| `assign` | `string $name, mixed $value` | Призначити змінну шаблону |
| `assignByRef` | `string $name, mixed &$value` | Призначити за посиланням |
| `append` | `string $name, mixed $value, bool $merge = false` | Додати до змінної масиву |
| `display` | `string $template` | Відобразити та вивести шаблон |
| `fetch` | `string $template` | Відобразити та повернути шаблон |
| `clearAssign` | `string $name` | Очистити призначену змінну |
| `clearAllAssign` | - | Очистити всі змінні |
| `getTemplateVars` | `string $name = null` | Отримати присвоєні змінні |
| `templateExists` | `string $template` | Перевірте, чи існує шаблон |
| `isCached` | `string $template` | Перевірте, чи шаблон кешується |
| `clearCache` | `string $template = null` | Очистити кеш шаблону |
...
1. **Увімкніть кешування шаблонів** у виробництві
2. **Призначайте лише необхідні змінні** - не передавайте цілі об’єкти
3. **Використовуйте модифікатори економно** - попередньо відформатуйте в PHP, коли це можливо
4. **Уникайте вкладених циклів** - реструктуруйте дані в PHP
5. **Кешуйте дорогі блоки** - використовуйте кешування блоків для складних запитів
