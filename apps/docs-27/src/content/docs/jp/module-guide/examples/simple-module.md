---
title: "シンプルモジュール例"
description: "すべての必要なファイルを備えた完全な動作モジュール"
---

# シンプルモジュール例 - ブログ

コアXOOPSコンセプトを示す、完全で動作するシンプルモジュール。
...
<h2>ブログ投稿を管理</h2>
<a href="admin.php?op=create" class="btn btn-primary">新規投稿</a>

<table class="table">
    {foreach from=$posts item=post}
        <tr>
            <td>{$post->post_title}</td>
            <td>{if $post->post_published}公開中{else}下書き{/if}</td>
            <td>
                <a href="admin.php?op=edit&id={$post->post_id}">編集</a>
                <a href="admin.php?op=delete&id={$post->post_id}">削除</a>
            </td>
        </tr>
    {/foreach}
</table>
        ...
        <div class="posts-list">
            {foreach from=$posts item=post}
                <article class="post">
                    ...
                    <small>投稿日: {$post->post_created|date_format:"%Y-%m-%d"}</small>
                    <p>{$post->post_content|truncate:200|escape}</p>
                    <a href="?op=view&id={$post->post_id}">続きを読む</a>
                </article>
            {/foreach}
        </div>
    {else}
...
<article class="blog-post">
    ...
    <small>投稿日: {$post->post_created|date_format:"%Y-%m-%d %H:%M"}</small>
...
- ページネーション付きで公開済みブログ投稿を表示
- 個別の投稿を表示
- 投稿を作成、編集、削除するための管理インターフェース
- ドラフト/公開ステータス
- タイムスタンプ

## テスト

1. XOOPS管理経由でモジュールをインストール
2. `modules/blog/` でフロントエンドにアクセス
3. `modules/blog/admin.php` で管理を訪問
4. テスト投稿を作成して公開

## 関連ドキュメント

関連トピック:
- ../Patterns/MVC-Pattern - パターン
- ../Patterns/Repository-Pattern - データアクセス
- ../Best-Practices/Code-Organization - 構造

---

タグ: #examples #simple-module #complete-example #module-development
