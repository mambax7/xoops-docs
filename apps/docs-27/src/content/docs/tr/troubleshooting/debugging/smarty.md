---
title: "Smarty template Hata Ayıklama"
description: "XOOPS için gelişmiş template hata ayıklama ve sorun giderme teknikleri"
---
> XOOPS temaları ve modüllerindeki Smarty şablonlarında hata ayıklamaya yönelik gelişmiş teknikler.
...
## Smarty Hata Ayıklama Modunu Etkinleştir

### Yöntem 1: Yönetici Paneli

XOOPS Yönetici > Ayarlar > Performans:
- "Hata Ayıklama Çıkışı"nı etkinleştirin
- "Hata Ayıklama Düzeyi"ni 2'ye ayarlayın

---

### Yöntem 2: Kod Yapılandırması
```php
<?php
...
### Yöntem 3: Tarayıcıda Açılır Pencerede Hata Ayıklama
```smarty
{* Add to template to enable debug in footer *}
{debug}
```
Bu, atanan tüm değişkenleri içeren bir açılır pencere gösterir.

---

## Ortak Smarty Hata Ayıklama Teknikleri

### Tüm Değişkenleri Dökümü
```php
<?php
// In PHP code
$tpl = new XoopsTpl();
...
### Şablondaki Değişkeni Kontrol Edin
```smarty
...
## Derlenmiş Şablonları Görüntüle

Smarty, performans için şablonları PHP'ye derler. Derlenmiş kodu görüntüleyerek hata ayıklayın:
```bash
...
## template Derlemesini Analiz Et
```php
<?php
...
## Belirli Sorunlarda Hata Ayıklama

### Sayı 1: Değişken Boş Olarak Gösteriliyor
```php
<?php
$tpl = new XoopsTpl();
...
### Sorun 2: Dizi Anahtarı Bulunamadı
```smarty
...
### Sayı 3: Plugin/Modifier Bulunamadı
```php
<?php
...
?>
```
### cache Etkinliğini Kontrol Edin
```php
<?php
$compile_dir = XOOPS_CACHE_PATH . '/smarty_compile';
...
## Hata Ayıklama Yardımcı İşlevleri
```php
<?php
...
#xoops #templates #Smarty #hata ayıklama #sorun giderme