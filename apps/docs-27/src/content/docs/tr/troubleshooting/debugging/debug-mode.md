---
title: "Hata Ayıklama Modunu Etkinleştir"
description: "Sorun giderme için XOOPS hata ayıklama modu nasıl etkinleştirilir ve kullanılır?"
---
> XOOPS hata ayıklama özellikleri ve araçlarına ilişkin kapsamlı kılavuz.

---

## Mimaride Hata Ayıklama
```mermaid
graph TB
...
## XOOPS Hata Ayıklama Düzeyleri

### Mainfile.php'de etkinleştirin
```php
<?php
...
| Seviye | PHP Hatalar | SQL Sorgular | template Değişkenleri | Tavsiye Edilenler |
|----------|---------------|---------------|---------------|------|
| 0 | Gizli | Hayır | Hayır | Üretim |
| 1 | Görüntülendi | Hayır | Hayır | Hızlı kontroller |
| 2 | Görüntülendi | Kayıtlı | Hayır | Geliştirme |
| 3 | Görüntülendi | Kayıtlı | Görüntülendi | Derin hata ayıklama |
...
## SQL Sorgu Hata Ayıklama

### Sorguları Hata Ayıklama Modunda Görüntüleme

`XOOPS_DEBUG_LEVEL` 2 veya 3'e ayarlandığında, sayfaların alt kısmında SQL sorguları görünür.

### Manuel Sorgu Günlüğü
```php
// Log specific query
...
### Etkinleştir Smarty Hata Ayıklama Konsolu
```php
...
## Ray Hata Ayıklayıcı Entegrasyonu
...
ray()->pause();
```
### database Sorgularında Hata Ayıklama
```php
// Log all queries
ray()->showQueries();
...
#xoops #hata ayıklama #sorun giderme #geliştirme #günlüğe kaydetme