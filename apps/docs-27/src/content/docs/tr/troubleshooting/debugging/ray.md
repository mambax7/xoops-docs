---
title: "Ray Hata Ayıklayıcısını Kullanma"
description: "Ray hata ayıklayıcı ile XOOPS uygulamalarında hata ayıklama"
---
# XOOPS için Ray Debugger'ı kullanma

> Ray ile modern hata ayıklama: XOOPS uygulamanızda değişkenleri inceleyin, mesajları kaydedin, SQL sorgularını izleyin ve profil performansını izleyin.

---

## Ray nedir?

Ray, yürütmeyi durdurmadan veya kesme noktalarını kullanmadan uygulama durumunu incelemenize yardımcı olan hafif bir hata ayıklama aracıdır. XOOPS geliştirme için mükemmeldir.

**Özellikler:**
- Mesajları ve değişkenleri günlüğe kaydet
- SQL sorgularını inceleyin
- Performansı takip edin
- Profil kodu
- Grupla ilgili günlükler
- Görsel zaman çizelgesi

**Gereksinimler:**
- PHP 7,4+
- Ray uygulaması (ücretsiz sürümü mevcut)
- Composer
...
```
### Adım 2: Ray Uygulamasını İndirin

[ray.so](https://ray.so) adresinden indirin:
- Mac: Ray.app
- Windows: Ray.exe
- Linux: ışın (AppImage)

### 3. Adım: Güvenlik Duvarını Yapılandırın (gerekirse)

Ray varsayılan olarak 23517 numaralı bağlantı noktasını kullanır:
```bash
# UFW
...
?>
```
### 2. Performans Profili Oluşturma
```php
<?php
$ray = ray();
...
?>
```
### 4. İlgili Günlüklerin Gruplandırılması
```php
<?php
$ray = ray();
...
## XOOPS-Özel Hata Ayıklama

### module Hata Ayıklama
```php
<?php
...
### module Kurulumunda Hata Ayıklama
```php
<?php
...
**C:**
1. Ray uygulamasının çalışıp çalışmadığını kontrol edin
2. Güvenlik duvarının 23517 numaralı bağlantı noktasına izin verdiğini kontrol edin
3. Ray'in kurulu olduğunu doğrulayın:
```bash
composer require spatie/ray
```
### S: SQL sorgularını göremiyorum
...
?>
```
### S: Ray'in performansa etkisi

**C:** Ray'in ek yükü minimum düzeydedir. Üretim için Ray çağrılarını kaldırın veya şunları devre dışı bırakın:
```php
<?php
...
#xoops #hata ayıklama #ray #profil oluşturma #izleme