---
title: "Publisher - Özel templates"
description: "Publisher şablonlarını özelleştirme ve özel themes oluşturma kılavuzu"
---
# Publisher'daki Özel templates

> Smarty, CSS ve HTML geçersiz kılmaları kullanarak Publisher şablonları oluşturma ve özelleştirme kılavuzu.

---

## template Sistemine Genel Bakış

### templates Nedir?

templates, Publisher'ın içeriği nasıl görüntüleyeceğini kontrol eder:
```
Templates render:
  ├── Article display
  ├── Category listings
...
### Şablonu Temaya Kopyala

**Yöntem 1: Dosya Yöneticisi aracılığıyla**
```
...
```
### Özel Şablonu Düzenle

theme kopyasını metin düzenleyicide açın:
```
...
## Smarty template Temelleri

### Smarty Değişkenler

Publisher şablonlara değişkenler sağlar:
...
{/if}
```
### Kategori Listeleme Şablonu

**Dosya: yayıncı_kategori.tpl**
```smarty
<!-- Category Page -->
...
```
### Son Makaleler Blok Şablonu

**Dosya: yayıncı_block_latest.tpl**
```smarty
<!-- Latest Articles Block -->
...
Temada özel CSS oluşturun:
```
/themes/yourtheme/modules/publisher/css/custom.css
```
### Temel template Yapısı

HTML yapısını anlayın:
```html
<!-- Publisher Module -->
...
## template Değişkenleri Referansı
...
## Gelişmiş template Teknikleri

### Diğer Şablonları Ekle
```smarty
...
### Görüntüle Smarty Değişkenler

Hata ayıklama için (üretimde kaldır):
```smarty
{* Show variable value *}
...
`/modules/publisher/xoops_version.php` veya yönetici ayarlarında:
```php
// Enable debug
...
### Eski Publisher Sürümünden

Eski sürümden yükseltme yapıyorsanız:

1. Eski ve yeni template dosyalarını karşılaştırın
2. Özel değişiklikleri birleştirin
3. Yeni değişken adlarını kullanın
4. İyice test edin
5. Eski şablonları yedekleyin
...
- [Smarty Belgeler](https://www.smarty.net/documentation)
- [Publisher GitHub](https://github.com/XoopsModules25x/publisher)
- [XOOPS template Kılavuzu](../../02-Core-Concepts/Templates/Smarty-Basics.md)

---

#Publisher #templates #Smarty #özelleştirme #theme oluşturma #xoops