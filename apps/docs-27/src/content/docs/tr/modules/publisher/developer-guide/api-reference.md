---
title: "Publisher - API Referans"
description: "Publisher modülü için API referansını sınıflar, yöntemler ve kod örnekleriyle tamamlayın"
---
# Publisher API Referans

> Publisher modülü sınıfları, yöntemleri, işlevleri ve API uç noktaları için eksiksiz referans.
...
`Item` sınıfı, Publisher'da tek bir article/item'yi temsil eder.

**Ad alanı:** `XoopsModules\Publisher\`

**Dosya:** `modules/publisher/class/Item.php`
...
`ItemHandler`, makaleler için CRUD işlemlerini yönetir.

**Dosya:** `modules/publisher/class/ItemHandler.php`
...
`Category` sınıfı bir kategoriyi veya bölümü temsil eder.

**Dosya:** `modules/publisher/class/Category.php`
...
## Kategoriİşleyici Sınıfı

### Genel Bakış

`CategoryHandler`, CRUD kategorisi işlemlerini yönetir.

**Dosya:** `modules/publisher/class/CategoryHandler.php`
...
Helper sınıfı yardımcı işlevler sağlar:

**Dosya:** `modules/publisher/class/Helper.php`
```php
// Get helper instance
...
### Ön Uç JavaScript İşlevler

Publisher, ön uç etkileşimleri için JavaScript API'yi içerir:
```javascript
...
Yayımcı, ön uç etkileşimleri için AJAX uç noktaları sağlar:
```javascript
...
Publisher REST API'yi gösterirse:
```
...
## Etkinlikler ve hooks

### Publisher Etkinlikleri
```php
// Item created event
...
?>
```
### Makaleyi Programlı Olarak Oluşturun
```php
<?php
// Create article
$itemHandler = xoops_getModuleHandler('Item', 'publisher');
...
- Publisher'daki templates ve Bloklar
- Makale Oluşturma
- Kategori Yönetimi
...
- [PHP Belgeler](https://www.php.net/docs.php)

---

#Publisher #api #referans #kod #sınıflar #yöntemler #xoops