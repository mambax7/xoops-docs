---
title: "Publisher - hooks ve Etkinlikler"
description: "hooks, etkinlikler ve eklenti sistemi kullanarak Publisher'ı genişletme kılavuzu"
---
# Publisher Kancaları ve Etkinlikleri

> Etkinlikleri, kancaları ve eklentileri kullanarak Publisher işlevselliğini genişletmeye yönelik eksiksiz kılavuz.
...
Etkinlikler, diğer modüllerin Publisher eylemlerine tepki vermesine olanak tanır:
```
...
### Öğe (Makale) Etkinlikleri

#### Publisher.item.created

Yeni bir makale oluşturulduğunda tetiklenir.
```php
...
Bir makale güncellendiğinde tetiklenir.
```php
...
Bir makale silindiğinde tetiklenir.
```php
...
Makale durumu yayınlandı olarak değiştiğinde tetiklenir.
```php
...
Bekleyen makale onaylandığında kovuldu.
```php
...
Makale reddedildiğinde tetiklenir.
```php
...
```
### Kategori Etkinlikleri

#### Publisher.kategori.oluşturuldu

Kategori oluşturulduğunda tetiklenir.
```php
...
```
#### Publisher.kategori.güncellendi

Kategori güncellendiğinde tetiklendi.
```php
...
```
#### Publisher.kategori.deleted

Kategori silindiğinde tetiklenir.
```php
...
Yorum yayınlandığında tetiklendi.
```php
...
Yorum onaylandığında tetiklenir.
```php
...
### Etkinlik İşleyiciyi Kaydet

Modülünüzde veya eklentinizde:
```php
<?php
...
### Örnek 1: Makale Oluşturma Konusunda E-posta Gönderin
```php
<?php
// Listen for article creation
xoops_events()->attach(
...
?>
```
### Örnek 2: Arama Dizinini Güncelleyin
```php
<?php
...
?>
```
### Örnek 3: Sosyal Medyaya Otomatik Gönderim
```php
<?php
...
?>
```
### Örnek 4: Harici Sistemle Senkronizasyon
```php
<?php
...
hooks, Publisher davranışında değişiklik yapılmasına izin verir:

#### Publisher.view.article.start

Makale işlenmeden önce çağrılır.
```php
xoops_events()->attach(
    'publisher.view.article.start',
...
Makale işlendikten sonra çağrılır.
```php
xoops_events()->attach(
    'publisher.view.article.end',
    'append_to_article'
...
permissions kontrol edilirken çağrılır.
```php
xoops_events()->attach(
    'publisher.permission.check',
...
Eklentiler Publisher'ın işlevselliğini genişletir:
...
Filtreler verileri değiştirir before/after işleniyor:
```php
<?php
...
Kodu belirli noktalarda yürütün:
```php
<?php
// Do action
...
### Örnek Eklenti: İlgili Makaleler
```php
<?php
...
### Etkinlik Dinleyici Yönergeleri
```php
...
- [XOOPS Etkinlik Sistemi](../../03-Module-Development/Module-Development.md)
- [Eklenti Geliştirme](../../03-Module-Development/Module-Development.md)

---

#Publisher #hooks #etkinlikler #eklentiler #uzantılar #özelleştirme #xoops