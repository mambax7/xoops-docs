---
title: "XoopsModule API Referans"
description: "XoopsModule ve module sistemi sınıfları için API referansını tamamlayın"
---
> XOOPS module sistemi için API belgelerini tamamlayın.

---

## module Sistem Mimarisi
```mermaid
graph TB
...
| Emlak | Tür | Açıklama |
|----------|------|------------|
| `mid` | int | module Kimliği |
| `name` | dize | Görünen ad |
| `version` | dize | Sürüm numarası |
| `dirname` | dize | Dizin adı |
| `isactive` | int | Aktif durum (0/1) |
| `hasmain` | int | Ana alanı var |
| `hasadmin` | int | Yönetici alanı var |
| `hassearch` | int | Arama fonksiyonu var |
| `hasconfig` | int | Yapılandırması var |
| `hascomments` | int | Yorumları var |
| `hasnotification` | int | Bildirimleri var |
