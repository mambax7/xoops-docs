---
title: "database Katmanı"
description: "XOOPS database soyutlaması, XoopsObject, işleyiciler ve Kriterler sistemine ilişkin kapsamlı kılavuz"
---
# 🗄️ database Katmanı

<span class="version-badge version-25x">2.5.x ✅</span> <span class="version-badge version-40x">4.0.x ✅</span>

> XOOPS database soyutlamasını, nesne kalıcılığını ve sorgu oluşturmayı anlama.

:::tip[Veri Erişiminizi Geleceğe Hazırlayın]
handler/Criteria modeli her iki versiyonda da çalışır. XOOPS 4.0'a hazırlanmak için, daha iyi test edilebilirlik amacıyla işleyicileri [Depo sınıfları](../../03-Module-Development/Patterns/Repository-Pattern.md) içine sarmayı düşünün. Bkz. [Veri Erişim Modeli Seçme](../../03-Module-Development/Choosing-Data-Access-Pattern.md).
:::
...
XOOPS database katmanı, MySQL/MariaDB, üzerinde aşağıdaki özelliklere sahip sağlam bir soyutlama sağlar:

- **Fabrika Modeli** - Merkezi database bağlantı yönetimi
- **Nesne-İlişkisel Eşleme** - XoopsObject ve işleyiciler
- **Sorgu Oluşturma** - Karmaşık sorgular için kriter sistemi
- **Bağlantının Yeniden Kullanımı** - Tek fabrika aracılığıyla tek bağlantı (havuz oluşturma değil)
...
Fabrika modeli tek bir database bağlantısının yeniden kullanılmasını sağlar:
```php
<?php
...
| `XOBJ_DTYPE_TXTBOX` | Dize | Kısa metin (< 255 karakter) |
| `XOBJ_DTYPE_TXTAREA` | Metin | Uzun metin içeriği |
| `XOBJ_DTYPE_EMAIL` | E-posta | E-posta adresleri |
| `XOBJ_DTYPE_URL` | URL | Web adresleri |
| `XOBJ_DTYPE_FLOAT` | Şamandıra | Ondalık sayılar |
| `XOBJ_DTYPE_ARRAY` | Dizi | Serileştirilmiş diziler |
| `XOBJ_DTYPE_OTHER` | Karışık | Ham veriler |
...
İşleyici sınıfı, XoopsObject örnekleri için CRUD işlemlerini yönetir.
```php
<?php
...
Criteria sistemi, SQL WHERE cümleleri oluşturmak için güçlü, nesne yönelimli bir yol sağlar.
...
```
### CriteriaCompo (Kriterleri Birleştirme)
```php
$criteria = new \CriteriaCompo();
...
Kriterlerle mümkün olmayan karmaşık sorgular için doğrudan SQL kullanın.

### Güvenli Sorgular (Okuma)
```php
$db = \XoopsDatabaseFactory::getDatabaseConnection();
...
## ⚠️ En İyi Güvenlik Uygulamaları

### Her Zaman user Girişinden Kaçış
```php
// NEVER do this
...
```
### Parametreli Sorguları Kullan (XMF)
```php
...
- [Kriter Sisteminin Ayrıntılı İncelemesi](../../04-API-Reference/Kernel/Criteria.md)
- [Tasarım Desenleri - Fabrika](../Architecture/Design-Patterns.md)
- [SQL Enjeksiyon Önleme](../Security/SQL-Injection-Prevention.md)
- [XoopsDatabase API Referans](../../04-API-Reference/Database/XoopsDatabase.md)
