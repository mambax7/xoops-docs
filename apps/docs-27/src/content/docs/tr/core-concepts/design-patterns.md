---
title: "XOOPS'deki Tasarım Desenleri"
description: "MVC, Singleton, Factory, Observer ve daha fazlasını içeren XOOPS geliştirmesinde kullanılan tasarım modellerine ilişkin kapsamlı kılavuz"
---
...
:::note[Hızlı Desen Seçimi]
Hangi modeli kullanacağınızdan emin değil misiniz? Bakınız:
- [Veri Erişim Modeli Seçme](../../03-Module-Development/Choosing-Data-Access-Pattern.md) — işleyiciler vs depo vs hizmet vs CQRS
- [Bir Etkinlik Sistemi Seçme](../Choosing-Event-System.md) — ön yüklemeler ve PSR-14 etkinlikleri
:::

## Genel Bakış

Bakımı yapılabilir XOOPS modülleri oluşturmak için tasarım modellerini anlamak ve doğru şekilde uygulamak çok önemlidir. Bu kılavuz, XOOPS geliştirmesinde en sık kullanılan kalıpları kapsar.
...
MVC modeli, bir uygulamayı birbirine bağlı üç bileşene ayırarak kod tabanını daha organize ve test edilebilir hale getirir.
...
Singleton modeli, bir sınıfın yalnızca bir örneğe sahip olmasını sağlar ve ona küresel erişim sağlar.
...
Fabrika modeli, nesneleri tam sınıflarını belirtmeden oluşturarak esnek nesne oluşturmaya olanak tanır.

### Ne Zaman Kullanılmalı

- Dinamik olarak işleyiciler oluşturma
...
Gözlemci modeli, nesnelerin bir öznenin durumundaki değişikliklerden haberdar edilmesini sağlayarak olaya dayalı davranışı mümkün kılar.
...
```
### XOOPS Ön Yüklemeler (Observer Uygulaması)
```php
<?php
...
Dekoratör modeli, aynı sınıftaki diğer nesneleri etkilemeden nesnelere dinamik olarak davranış ekler.

### Ne Zaman Kullanılmalı

- Form öğesi özelleştirmesi
...
Depo modeli, veri erişim mantığı ile iş mantığı arasında bir soyutlama katmanı sağlar.

### Ne Zaman Kullanılmalı

- Karmaşık veri erişim gereksinimleri
...
Bağımlılık Enjeksiyonu (DI), nesnelerin dahili olarak oluşturulması yerine bağımlılıklarıyla oluşturulmasına olanak tanır.
...
### Desen Seçimi Yönergeleri

1. **Beklenenleri değil, gerçek ihtiyaçları temel alan kalıpları seçin**
2. **Uygulamaları basit tutun** - aşırı mühendislik yapmayın
3. Ekibin anlaşılması için **belge modeli kullanımı**
4. **Uygun olduğunda desenleri birleştirin** (ör. Fabrika + Tekil)
5. Desenleri seçerken **test edilebilirliği göz önünde bulundurun**
...
- [XOOPS-Mimarlık](XOOPS-Architecture.md) - Genel sistem mimarisi
- [database Katmanı](../Database/Database-Layer.md) - Veri kalıcılık modelleri
- [En İyi Güvenlik Uygulamaları](../Security/Security-Best-Practices.md) - Güvenli model uygulaması
