---
title: "İzin Sistemi"
description: "XOOPS izin sistemi, türleri, kontrolü, hiyerarşisi ve erişim kontrolüne ilişkin eksiksiz kılavuz"
---
# XOOPS'deki İzin Sistemi

XOOPS İzin Sistemi, kimin hangi kaynaklar üzerinde hangi eylemleri gerçekleştirebileceğini yöneten ayrıntılı bir erişim kontrolü çerçevesidir. Bu belge izin türlerini, kontrol mekanizmalarını, hiyerarşiyi ve uygulama örneklerini kapsar.

## İzin Türleri

### module Düzeyinde permissions

module düzeyindeki permissions, tüm modüllere veya module işlevlerine erişimi denetler.

**Ortak İzin Adları:**
- `module_view` - module içeriğini görüntüle
- `module_read` - module kaynaklarını okuyun
- `module_submit` - İçeriği modüle gönder
- `module_edit` - module içeriğini düzenle
- `module_admin` - Yönetim modülü
```php
<?php
/**
...
```
### Öğe Düzeyinde permissions

Öğe düzeyindeki permissions, bir module içindeki belirli kaynaklara erişimi denetler.

**Örnekler:**
- Makale Kimliği: view/edit belirli bir makaleyi gruplandırabilir misiniz?
- Kategori Kimliği: Grup kategoriye erişebilir mi?
- Sayfa Kimliği: view/modify belirli bir sayfayı gruplayabilir mi?
```php
<?php
/**
...
Blok izinleri, sayfalarda görüntülenen bloklarla görünürlüğü ve etkileşimi kontrol eder.
```php
<?php
/**
...
Grup yönetimini ve yönetimini kontrol eden permissions.
```php
<?php
/**
...
`XoopsGroupPermHandler` sınıfı, izinleri kontrol etmek ve yönetmek için yöntemler sağlar.
```php
<?php
/**
...
```
## İzin Kontrolü Uygulaması

### Tek user İzin Kontrolü
```php
<?php
/**
...
### Optimize Edilmiş İzin Kontrolü
```php
<?php
/**
...
### İzin Atama Kuralları

1. **En Az Ayrıcalık Prensibi**: Yalnızca gerekli izinleri atayın
2. **Rol Tabanlı Erişim**: Rol tabanlı permissions için grupları kullanın
3. **Düzenli Denetimler**: İzinlerin periyodik olarak gözden geçirilmesi
4. **Görev Ayrılığı**: Yöneticiyi user izinlerinden ayırın
5. **Açık Reddetme**: Varsayılan reddetme, açık izin verme yaklaşımı
...
#permissions #erişim kontrolü #güvenlik #yetkilendirme #acl #izin kontrolü