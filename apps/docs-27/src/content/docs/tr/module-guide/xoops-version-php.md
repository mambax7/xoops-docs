---
title: "xoops_version.php - module Bildirimi"
---
## Genel Bakış

`xoops_version.php` dosyası her XOOPS modülünün kalbidir. module meta verilerini, database tablolarını, şablonları, blokları, yapılandırma seçeneklerini ve kurulum kancalarını tanımlar.
...
|-----|------|------------|
| `name` | dize | Görünen ad (dil sabitini kullanın) |
| `version` | dize | Anlamsal sürüm (MAJOR.MINOR.PATCH) |
| `description` | dize | module açıklaması |
| `author` | dize | Birincil yazarın adı |
| `credits` | dize | Ek katkıda bulunanlar |
| `license` | dize | Lisans adı |
...
| form türü | değer türü | Açıklama |
|----------|-----------|------------|
| `textbox` | `text`/`int` | Tek hatlı giriş |
| `textarea` | `text` | Çok hatlı giriş |
| `yesno` | `int` | Yes/No radyo |
| `select` | `text` | Açılır menü seçimi |
| `select_multi` | `array` | Çoklu seçim |
| `group` | `int` | Grup seçici |
| `group_multi` | `array` | Çoklu grup seçici |
| `user` | `int` | user seçici |
| `color` | `text` | Renk seçici |
| `hidden` | `text` | Gizli alan |
...
- module Geliştirme - Tam module kılavuzu
- module Yapısı - Dizin yapısı
- Blok Geliştirme - Blok oluşturma
- database İşlemleri - database kurulumu