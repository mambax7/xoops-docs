---
title: "Smarty template Kuralları"
description: "XOOPS Smarty template kodlama standartları ve en iyi uygulamalar"
---
> XOOPS, template oluşturmak için Smarty'yı kullanır. Bu kılavuz, Smarty şablonlarının geliştirilmesine yönelik kuralları ve en iyi uygulamaları kapsar.

---

## Genel Bakış

XOOPS Smarty şablonları aşağıdaki gibidir:

- **XOOPS template yapısı** ve adlandırma
- **Erişilebilirlik standartları** (WCAG)
- **Semantik HTML5** işaretlemesi
- **BEM tarzı sınıf adlandırma**
...
<{/foreach}>
```
### Etiketlerin Etrafında Boşluk Bırakma
```smarty
...
<{/if}>
```
### Bölüm (kullanımdan kaldırıldı, bunun yerine foreach kullanın)
```smarty
...
- Semantik kullanın HTML5
- Resimlere alternatif metin ekleyin
- Metin için dil sabitlerini kullanın
- Çıkış çıkışı (varsayılan)
- Mantığı minimumda tutun
- Anlamlı değişken adları kullanın
- Dosya başlıklarını dahil et
- BEM tarzı sınıf adlarını kullanın
- Ekran okuyucularla test edin

### Yapma

- Mantık ve sunumu karıştırmayın
- Alternatif metni unutmayın
- Kaçmadan raw HTML kullanmayın
- Şablonlarda global değişkenler oluşturmayın
- Kullanımdan kaldırılan Smarty özelliklerini kullanmayın
- Şablonları çok derin bir şekilde iç içe yerleştirmeyin
- Erişilebilirliği göz ardı etmeyin
- Metni sabit kodlamayın (sabitleri kullanın)

---

## template Örnekleri

### Komple module Şablonu
```smarty
{*
...
#xoops #Smarty #templates #uzlaşımlar #en iyi uygulamalar