---
title: "Penerbit - Cangkuk dan Acara"
description: "Panduan untuk melanjutkan Penerbit menggunakan cangkuk, acara dan sistem pemalam"
---
# Pengait dan Acara Penerbit

> Panduan lengkap untuk melanjutkan fungsi Penerbit menggunakan acara, cangkuk dan pemalam.

---

## Gambaran Keseluruhan Sistem Acara

### Apakah Peristiwa?

Acara membenarkan modul lain bertindak balas terhadap tindakan Penerbit:
```
...
### Perkara (Artikel) Acara

#### publisher.item.created

Dipecat apabila artikel baharu dibuat.
```php
...
Dipecat apabila artikel dikemas kini.
```php
...
Dipecat apabila artikel dipadamkan.
```php
...
Dipecat apabila status artikel bertukar kepada diterbitkan.
```php
...
Dipecat apabila artikel yang belum selesai diluluskan.
```php
...
Dipecat apabila artikel ditolak.
```php
...
#### penerbit.kategori.dicipta

Dipecat apabila kategori dicipta.
```php
...
Dipecat apabila kategori dikemas kini.
```php
...
Dipecat apabila kategori dipadamkan.
```php
...
Dipecat apabila ulasan disiarkan.
```php
...
Dipecat apabila ulasan diluluskan.
```php
...
Dipecat apabila ulasan dipadamkan.
```php
...
### Contoh 1: Hantar E-mel pada Penciptaan Artikel
```php
<?php
// Listen for article creation
xoops_events()->attach(
...
?>
```
### Contoh 2: Kemas Kini Indeks Carian
```php
<?php
...
?>
```
### Contoh 3: Auto-Siar ke Media Sosial
```php
<?php
...
?>
```
### Contoh 4: Segerakkan dengan Sistem Luaran
```php
<?php
...
Hooks membenarkan pengubahsuaian pada tingkah laku Penerbit:

#### publisher.view.article.start

Dipanggil sebelum artikel diberikan.
```php
xoops_events()->attach(
    'publisher.view.article.start',
...
Dipanggil selepas artikel diberikan.
```php
xoops_events()->attach(
    'publisher.view.article.end',
    'append_to_article'
...
Dipanggil semasa menyemak kebenaran.
```php
xoops_events()->attach(
    'publisher.permission.check',
...
Pemalam memanjangkan fungsi Penerbit:
...
Penapis mengubah suai data before/after pemprosesan:
```php
<?php
...
Laksanakan kod pada titik tertentu:
```php
<?php
// Do action
...
## Melanjutkan dengan Pemalam

### Contoh Plugin: Artikel Berkaitan
```php
<?php
...
- [GitHub Penerbit](https://github.com/XoopsModules25x/publisher)
- [XOOPS Sistem Acara](../../03-Module-Development/Module-Development.md)
- [Pembangunan Plugin](../../03-Module-Development/Module-Development.md)
