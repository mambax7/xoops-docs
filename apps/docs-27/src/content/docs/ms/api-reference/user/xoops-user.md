---
title: "Rujukan API XoopsUser"
description: "Rujukan API lengkap untuk kelas pengurusan XoopsUser dan pengguna"
---
> Dokumentasi API lengkap untuk sistem pengguna XOOPS.---

## Seni Bina Sistem Pengguna
```
mermaid
classDiagram
    class XoopsUser {
...
## Kelas XoopsUser### Hartanah| Hartanah | Taip | Penerangan |
|----------|------|-------------|
| `uid` | int | ID Pengguna (kunci utama) |
| `uname` | rentetan | Nama pengguna |
| `name` | rentetan | Nama sebenar |
| `email` | rentetan | Alamat e-mel |
| `pass` | rentetan | Cincang kata laluan |
| `url` | rentetan | URL tapak web |
| `user_avatar` | rentetan | Nama fail Avatar |
| `user_regdate` | int | Cap masa pendaftaran |
| `user_from` | rentetan | Lokasi |
| `user_sig` | rentetan | Tandatangan |
| `user_occ` | rentetan | Pekerjaan |
| `user_intrest` | rentetan | Minat |
| `bio` | rentetan | Biografi |
| `posts` | int | Kiraan siaran |
| `rank` | int | Kedudukan pengguna |
| `level` | int | Tahap pengguna (0=tidak aktif, 1=aktif) |
| `theme` | rentetan | Tema pilihan |
| `timezone` | terapung | Zon waktu mengimbangi |
| `last_login` | int | Cap masa log masuk terakhir |### Kaedah Teras
```
php
// Get current user
global $xoopsUser;
...
## XoopsUserHandler### Operasi CRUD Pengguna
```
php
// Get handler
...
## XoopsMemberHandler### Pengurusan Kumpulan
```
php
...
## Sistem Kumpulan### Kumpulan Lalai| ID Kumpulan | Nama | Penerangan |
|----------|------|-------------|
| 1 | Juruweb | Akses pentadbiran penuh |
| 2 | Pengguna Berdaftar | Pengguna berdaftar standard |
| 3 | Tanpa Nama | Pelawat tidak log masuk |### Kebenaran Kumpulan
```
mermaid
graph TB
...
## Dokumentasi Berkaitan- Panduan Pengurusan Pengguna
- Sistem Kebenaran
- Pengesahan---

#XOOPS #api #pengguna #pengesahan #rujukan