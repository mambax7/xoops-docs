---
title: "xoops_version.php - Manifest del Modulo"
---

## Panoramica

Il file `xoops_version.php` è il cuore di ogni modulo XOOPS. Definisce i metadati del modulo, le tabelle del database, i template, i blocchi, le opzioni di configurazione e gli hook di installazione.
...
|-----|------|-------------|
| `name` | string | Nome visualizzato (usa costante di lingua) |
| `version` | string | Versione semantica (MAJOR.MINOR.PATCH) |
| `description` | string | Descrizione del modulo |
| `author` | string | Nome autore principale |
| `credits` | string | Contributori aggiuntivi |
| `license` | string | Nome licenza |
| `dirname` | string | Nome directory del modulo |
...
| formtype | valuetype | Descrizione |
|----------|-----------|-------------|
| `textbox` | `text`/`int` | Input single-line |
| `textarea` | `text` | Input multi-line |
| `yesno` | `int` | Radio Yes/No |
| `select` | `text` | Dropdown select |
| `select_multi` | `array` | Multi-select |
| `group` | `int` | Selettore di gruppo |
| `group_multi` | `array` | Selettore di gruppo multiplo |
| `user` | `int` | Selettore utente |
| `color` | `text` | Color picker |
| `hidden` | `text` | Campo nascosto |
...
- Module-Development - Guida completa del modulo
- Module-Structure - Struttura delle directory
- Block-Development - Creazione dei blocchi
- Database-Operations - Setup del database
