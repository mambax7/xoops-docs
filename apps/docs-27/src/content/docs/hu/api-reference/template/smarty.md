---
title: "Smarty Template API referencia"
description: "Teljes API-referencia a Smarty-sablonokhoz XOOPS-ban"
---
> Teljes API-dokumentáció a Smarty-sablonokhoz a XOOPS-ban.

---

## Sablon motor architektúrája
...
| Módszer | Paraméterek | Leírás |
|--------|-------------|--------------|
| `assign` | `string $name, mixed $value` | Változó hozzárendelése a sablonhoz |
| `assignByRef` | `string $name, mixed &$value` | Referencia alapján hozzárendelés |
| `append` | `string $name, mixed $value, bool $merge = false` | Hozzáfűzés a | tömbváltozóhoz
| `display` | `string $template` | Renderelési és kimeneti sablon |
| `fetch` | `string $template` | Renderelési és visszaküldési sablon |
| `clearAssign` | `string $name` | Hozzárendelt változó törlése |
| `clearAllAssign` | - | Minden változó törlése |
| `getTemplateVars` | `string $name = null` | Hozzárendelt változók lekérése |
| `templateExists` | `string $template` | Ellenőrizze, hogy létezik-e sablon |
| `isCached` | `string $template` | Ellenőrizze, hogy a sablon gyorsítótárban van-e |
| `clearCache` | `string $template = null` | Sablon gyorsítótár törlése |
...
1. **Sablon gyorsítótárazásának engedélyezése** éles környezetben
2. **Csak a szükséges változókat rendelje hozzá** – ne adjon át teljes objektumokat
3. **Használjon takarékosan módosítókat** – lehetőség szerint előre formázza a PHP formátumban
4. **Kerülje a beágyazott hurkokat** – az adatok átstrukturálása a PHP-ban
5. **Drága blokkok gyorsítótárazása** – összetett lekérdezésekhez használjon blokk-gyorsítótárat
...
#xoops #api #okos #sablonok #referencia
