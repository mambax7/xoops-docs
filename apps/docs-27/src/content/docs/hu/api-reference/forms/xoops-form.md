---
title: "XOOPSForm API referencia"
description: "Teljes API hivatkozás a XOOPSForm és űrlapelem osztályokhoz"
---
> Töltse ki a API dokumentációt a XOOPS űrlapgeneráló rendszerhez.
...
|--------|------------|----------|--------------|
| `addElement` | `XOOPSFormElement $element, bool $required = false` | `void` | Elem hozzáadása az űrlaphoz |
| `getElements` | - | `array` | Minden elem lekérése |
| `getElement` | `string $name` | `XOOPSFormElement|null` | Elem lekérése név szerint |
| `setExtra` | `string $extra` | `void` | További HTML attribútumok beállítása |
| `getExtra` | - | `string` | További attribútumok beszerzése |
| `getTitle` | - | `string` | Űrlap címének lekérése |
| `setTitle` | `string $title` | `void` | Állítsa be az űrlap címét |
| `getName` | - | `string` | Űrlapnév lekérése |
| `getAction` | - | `string` | Cselekvés kérése URL |
| `render` | - | `string` | Renderelési űrlap HTML |
| `display` | - | `void` | Echo renderelt forma |
| `insertBreak` | `string $extra = ''` | `void` | Vizuális törés beszúrása |
| `setRequired` | `XOOPSFormElement $element` | `void` | Szükséges elem megjelölése |

---

## XOOPSThemeForm

A leggyakrabban használt űrlaposztály, témájú stílusban jelenít meg.
...
Legördülő menü vagy többszörös kijelölés.
...
Jelölőnégyzet vagy jelölőnégyzetcsoport.
...
Küldés, visszaállítás vagy egyéni gomb.
...
Csak megjelenítési címke (nem bemenet).
...
#xoops #api #űrlapok #xoopsform #referencia
