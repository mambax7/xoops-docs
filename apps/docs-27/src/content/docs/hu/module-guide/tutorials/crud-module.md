---
title: "CRUD modul építése"
description: "Teljes oktatóanyag adatbázis-műveletekkel, űrlapokkal és adminisztrátori felülettel rendelkező CRUD-modul felépítéséhez"
---
# A CRUD modul oktatóanyagának elkészítése

Ez az oktatóanyag végigvezeti a teljes CRUD (létrehozás, olvasás, frissítés, törlés) modul felépítésén a XOOPS számára. Létrehozunk egy „Jegyzetek” modult, amely lehetővé teszi a felhasználók számára a személyes jegyzetek kezelését.

## Előfeltételek

- Befejezett Hello-World-module oktatóanyag
- A PHP OOP fogalmak megértése
- Alapvető SQL ismeretek

## modul áttekintése

**Megjegyzés modul jellemzői:**
- Jegyzetek létrehozása, megtekintése, szerkesztése és törlése
- Admin kezelő felület
- Felhasználó-specifikus megjegyzések
- Kategória szervezés
- Keresés funkció

## 1. lépés: Címtárszerkezet

Hozza létre a következő struktúrát a `/modules/notes/`-ban:
...
## 2. lépés: Adatbázisséma

`sql/mysql.sql` létrehozása:
...
## 4. lépés: Entitásosztályok

### Megjegyzés entitás

`class/Note.php` létrehozása:
...
`class/NoteHandler.php` létrehozása:
...
`class/Category.php` létrehozása:
...
`class/CategoryHandler.php` létrehozása:
...
## 5. lépés: Általános beépítési fájl

`include/common.php` létrehozása:
...
### Oldal szerkesztése (Create/Update)

`edit.php` létrehozása:
...
`templates/notes_index.tpl` létrehozása:
...
`templates/notes_view.tpl` létrehozása:
...
`templates/notes_edit.tpl` létrehozása:
...
## 9. lépés: Adminisztrációs felület
...
Gratulálunk! Ön egy komplett CRUD modult épített fel. Az érintett kulcsfogalmak:

1. **Adatbázis tervezés** - Táblázatok kapcsolatokkal
2. **Entitásosztályok** - XOOPSObject beírt tulajdonságokkal
3. **Handler osztályok** – XOOPSPersistableObjectHandler egyéni módszerekkel
4. **Frontend Pages** - Listázás, megtekintés és szerkesztés funkciók
5. **Űrlapkezelés** - CSRF védelem és érvényesítés
6. **Adminisztrációs felület** - Kezelőképernyők
7. **Sablonok** - Okos sablonok logikával

## Következő lépések

- Adjon hozzá további speciális funkciókat (megjegyzések, értékelések, megosztás)
- Valósítsa meg a ../Patterns/Repository-Pattern-t a tisztább adathozzáférés érdekében
- Alkalmazza a ../Patterns/MVC-Pattern-t a jobb kódszervezés érdekében
- Adja hozzá a ../Best-Practices/Testing-t a PHPUnit segítségével

Lásd még: ../module-Development | ../Patterns/MVC-Pattern | ../Patterns/Repository-Pattern
