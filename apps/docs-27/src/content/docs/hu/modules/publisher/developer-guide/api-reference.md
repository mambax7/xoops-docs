---
title: "Kiadó – API-referencia"
description: "Teljes API-referencia a Publisher modulhoz osztályokkal, metódusokkal és kódpéldákkal"
---
# Kiadói API hivatkozás

> Teljes referencia a Publisher modulosztályokhoz, metódusokhoz, függvényekhez és API végpontokhoz.
...
A `Item` osztály egyetlen article/item-t jelent a Publisherben.

**Névtér:** `XOOPSmodules\Publisher\`

**Fájl:** `modules/publisher/class/Item.php`
...
A `ItemHandler` kezeli a cikkekhez tartozó CRUD műveleteket.

**Fájl:** `modules/publisher/class/ItemHandler.php`

### Elemek visszakeresése
...
A `Category` osztály egy kategóriát vagy szakaszt jelent.

**Fájl:** `modules/publisher/class/Category.php`
...
## Kategóriakezelő osztály

### Áttekintés

A `CategoryHandler` kezeli a CRUD kategóriájú műveleteket.

**Fájl:** `modules/publisher/class/CategoryHandler.php`
...
A Helper osztály segédfunkciókat biztosít:

**Fájl:** `modules/publisher/class/Helper.php`
...
### Előtér JavaScript-funkciói

A kiadó tartalmazza a JavaScript API-t a frontend interakciókhoz:
...
A Publisher AJAX végpontokat biztosít a frontend interakciókhoz:
...
## REST API (ha engedélyezve van)

### API Végpontok

Ha a Kiadó közzéteszi a REST API-t:
...
### Cikk létrehozása programozottan
...
### Cikkek beszerzése kategóriák szerint
...
- Horgok és események
- Egyedi sablonok
- Kiadói modul elemzése
- Sablonok és blokkok a Publisherben
- Cikk létrehozása
- Kategóriakezelés
...
#kiadó #api #referencia #kód #osztályok #módszerek #xoops
