---
title: "Reference XoopsUser API"
description: "Kompletní reference API pro XoopsUser a třídy správy uživatelů"
---

> Kompletní dokumentace API pro uživatelský systém XOOPS.
...
|----------|------|-------------|
| `uid` | int | ID uživatele (primární klíč) |
| `uname` | řetězec | Uživatelské jméno |
| `name` | řetězec | Skutečné jméno |
| `email` | řetězec | E-mailová adresa |
| `pass` | řetězec | Hash hesla |
| `url` | řetězec | Webové stránky URL |
| `user_avatar` | řetězec | Název souboru avataru |
| `user_regdate` | int | Časové razítko registrace |
| `user_from` | řetězec | Umístění |
| `user_sig` | řetězec | Podpis |
| `user_occ` | řetězec | Povolání |
| `user_intrest` | řetězec | Zájmy |
| `bio` | řetězec | Životopis |
| `posts` | int | Počet příspěvků |
| `rank` | int | Uživatelské hodnocení |
| `level` | int | Uživatelská úroveň (0=neaktivní, 1=aktivní) |
| `theme` | řetězec | Preferovaný motiv |
| `timezone` | plovák | Posun časového pásma |
| `last_login` | int | Časové razítko posledního přihlášení |
...
| ID skupiny | Jméno | Popis |
|----------|------|-------------|
| 1 | Webmasteři | Úplný administrativní přístup |
| 2 | Registrovaní uživatelé | Standardní registrovaní uživatelé |
| 3 | Anonymní | Nepřihlášení návštěvníci |
...
- Příručka správy uživatelů
- Systém povolení
- Autentizace

---

#xoops #api #user #autentifikace #reference