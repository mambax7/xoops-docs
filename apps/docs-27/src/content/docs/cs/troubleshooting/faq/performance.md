---
title: "Výkon FAQ"
description: "Často kladené otázky týkající se optimalizace výkonu XOOPS"
---

# Často kladené otázky o výkonu

> Běžné otázky a odpovědi týkající se optimalizace výkonu XOOPS a diagnostiky pomalých stránek.

---

## Obecný výkon

### Otázka: Jak zjistím, zda je můj web XOOPS pomalý?

**Odpověď:** Použijte tyto nástroje a metriky:
...
2. **Cílové metriky**:
- První obsahové vybarvení (FCP): < 1,8 s
- Největší obsahová barva (LCP): < 2,5 s
- Čas do prvního bajtu (TTFB): < 0,6 s
- Celková velikost stránky: < 2-3 MB
...
### Otázka: Jaké jsou nejčastější problémy s výkonem?
...
### Otázka: Kam bych měl zaměřit své optimalizační úsilí?

**A:** Dodržujte prioritu optimalizace:
...
### Otázka: Jak povolím ukládání do mezipaměti v XOOPS?

**A:** XOOPS má vestavěné ukládání do mezipaměti. Konfigurace v části Správce > Nastavení > Výkon:
...
### Otázka: Jaký typ mezipaměti bych měl použít?

**A:**
- **File Cache**: Výchozí, jednoduchá, žádné další nastavení. Dobré pro malé weby.
- **Memcache**: Rychlejší, založený na paměti. Lepší pro stránky s vysokou návštěvností.
- **Redis**: Nejvýkonnější, podporuje více typů dat. Nejlepší pro škálování.
...
Poté povolte v XOOPS admin.

---

### Otázka: Jak vymažu mezipaměť XOOPS?
...
### Otázka: Jak dlouho mám ukládat data do mezipaměti?

**A:** Závisí na požadavcích na aktuálnost dat:
...
### Otázka: Jak najdu pomalé databázové dotazy?

**A:** Povolit protokolování dotazů:
...
Nebo použijte protokol pomalého dotazu MySQL: 
```bash
...
### Otázka: Jak mohu optimalizovat databázové dotazy?

**A:** Postupujte takto:

**1. Přidat databázové indexy**
```sql
...
**2. Použijte LIMIT a stránkování**
```php
<?php
...
**3. Vyberte pouze potřebné sloupce**
```php
<?php
// WRONG
$result = $db->query("SELECT * FROM xoops_articles");
...
**5. Použijte EXPLAIN k analýze dotazů**
```sql
...
### Otázka: Jak mohu snížit zatížení databáze?

**A:**
1. **Výsledky dotazu mezipaměti**:
```php
<?php
$cache = xoops_cache_handler::getInstance();
...
2. **Archivujte stará data** do samostatných tabulek
3. **Protokoly pravidelně čistěte**:
```bash
# Delete old log entries (older than 30 days)
DELETE FROM xoops_log WHERE created < NOW() - INTERVAL 30 DAY;
```

4. **Povolit mezipaměť dotazů** (MySQL):
```sql
...
### Otázka: Jak mohu optimalizovat CSS a JavaScript?

**A:**

**1. Minifikovat soubory**:
```bash
# Using online tools
# - cssminifier.com
...
**2. Kombinovat související soubory**:
```html
...
**3. Odložit nekritické JavaScript**:
```html
...
**4. Povolit kompresi Gzip** (.htaccess):
```apache
...
### Otázka: Jak mohu optimalizovat obrázky?

**A:**

**1. Vyberte správný formát**:
- JPG: Fotografie a složité obrázky
- PNG: Grafika a obrázky s průhledností
- WebP: Moderní prohlížeče, lepší komprese
- AVIF: Nejnovější, nejlepší komprese

**2. Komprimovat obrázky**:
```bash
# Using ImageMagick
...
**3. Poskytovat responzivní obrázky**:
```html
...
### Otázka: Jak zkontroluji výkon serveru?
...
### Otázka: Jak mohu optimalizovat PHP pro XOOPS?

**A:** Upravit `/etc/php/8.x/fpm/php.ini`:
...
### Otázka: Jak povolím HTTP/2 a kompresi?

**A:** Pro Apache (.htaccess):
```apache
...
### Otázka: Jak mohu sledovat výkon XOOPS v průběhu času?

**A:**

**1. Použijte Google Analytics**:
- Core Web Vitals
- Doba načítání stránky
- Chování uživatele

**2. Použijte Server Monitoring Tools**:
```bash
...
**3. Protokolovat a analyzovat požadavky**:
```bash
...
### Otázka: Jak zjistím úniky paměti?
...
#xoops #výkon #optimalizace #časté dotazy #odstraňování problémů #ukládání do mezipaměti