---
title: "CRUD 모듈 구축"
description: "데이터베이스 작업, 양식 및 관리 인터페이스를 갖춘 CRUD 모듈 구축을 위한 전체 튜토리얼"
---

# CRUD 모듈 튜토리얼 구축

이 튜토리얼에서는 XOOPS용 완전한 CRUD(생성, 읽기, 업데이트, 삭제) 모듈을 구축하는 과정을 안내합니다. 사용자가 개인 메모를 관리할 수 있는 "메모" 모듈을 만들겠습니다.

## 전제 조건

- Hello-World-모듈 튜토리얼 완료
- PHP OOP 개념의 이해
- 기본적인 SQL 지식
...
축하합니다! 완전한 CRUD 모듈을 구축했습니다. 다루는 주요 개념:

1. **데이터베이스 디자인** - 관계가 있는 테이블
2. **엔티티 클래스** - 유형화된 속성이 있는 XoopsObject
3. **핸들러 클래스** - 사용자 정의 메서드가 있는 XoopsPersistableObjectHandler
4. **프런트엔드 페이지** - 목록, 보기, 편집 기능
5. **양식 처리** - CSRF 보호 및 검증
6. **관리 인터페이스** - 관리 화면
7. **템플릿** - 논리가 포함된 Smarty 템플릿

## 다음 단계

- 고급 기능 추가(댓글, 평점, 공유)
- 더 깔끔한 데이터 액세스를 위해../Patterns/Repository-Pattern을 구현합니다.
- 더 나은 코드 구성을 위해../Patterns/MVC-Pattern 적용
-../모범 사례/PHPUnit을 사용한 테스트 추가

참조:../모듈 개발 |../패턴/MVC 패턴 |../패턴/저장소 패턴
