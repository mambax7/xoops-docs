---
title: "허가 시스템"
description: "XOOPS 권한 시스템, 유형, 확인, 계층 구조 및 액세스 제어에 대한 전체 가이드"
---

# XOOPS의 권한 시스템

XOOPS 권한 시스템은 누가 어떤 리소스에 대해 어떤 작업을 수행할 수 있는지 관리하는 세분화된 액세스 제어 프레임워크입니다. 이 문서에서는 권한 유형, 확인 메커니즘, 계층 구조 및 구현 예를 다룹니다.

## 권한 유형

### 모듈 수준 권한

모듈 수준 권한은 전체 모듈 또는 모듈 기능에 대한 액세스를 제어합니다.

**공통 권한 이름:**
- `module_view` - 모듈 내용 보기
- `module_read` - 모듈 리소스 읽기
- `module_submit` - 모듈에 콘텐츠 제출
- `module_edit` - 모듈 내용 편집
- `module_admin` - 모듈 관리
...
항목 수준 권한은 모듈 내의 특정 리소스에 대한 액세스를 제어합니다.

**예:**
- 기사 ID: 특정 기사를 그룹으로 보거나 편집할 수 있나요?
- 카테고리ID : 그룹이 카테고리에 접근할 수 있나요?
- 페이지 ID: 특정 페이지를 그룹으로 보거나 수정할 수 있나요?
...
차단 권한은 페이지에 표시되는 블록의 가시성과 상호 작용을 제어합니다.
...
`XoopsGroupPermHandler` 클래스는 권한을 확인하고 관리하는 방법을 제공합니다.
...
1. **최소 권한의 원칙**: 꼭 필요한 권한만 부여
2. **역할 기반 액세스**: 역할 기반 권한을 위해 그룹을 사용합니다.
3. **정기 감사**: 정기적으로 권한 검토
4. **업무 분리**: 관리자와 사용자 권한을 분리합니다.
5. **명시적 거부**: 기본 거부, 명시적 허용 접근 방식
...
#permissions #액세스 제어 #security #authorization #acl #permission-checking
