---
title: "사용자 관리"
description: "XOOPS 사용자 시스템, 등록, 인증 및 프로필 관리에 대한 종합 가이드"
---

# XOOPS의 사용자 관리

XOOPS 사용자 관리 시스템은 사용자 등록, 인증, 프로필 관리 및 사용자 기본 설정을 처리하기 위한 완벽한 프레임워크를 제공합니다. 이 문서에서는 사용자 개체 구조, 등록 흐름 및 실제 구현 예를 다룹니다.

## 사용자 개체 구조

XOOPS의 핵심 사용자 개체는 모든 사용자 데이터와 메서드를 캡슐화하는 `XoopsUser` 클래스입니다.
...
- 항상 `PASSWORD_BCRYPT` 알고리즘과 함께 `password_hash()`을 사용합니다.
- bcrypt에 비용 매개변수 12를 사용합니다.
- 일반 텍스트 비밀번호를 저장하지 마세요.
- 비밀번호 만료 정책 구현
- 손상된 계정에 대해 비밀번호 변경을 요구합니다.
