...
이 문서에서는 입력 유효성 검사, 출력 인코딩, 인증, 승인 및 일반적인 웹 취약점에 대한 보호를 다루는 XOOPS 개발을 위한 보안 모범 사례를 간략하게 설명합니다.
...
- [ ] 모든 사용자 입력이 검증되고 삭제되었습니다.
- [ ] 모든 데이터베이스 작업에 대한 매개변수화된 쿼리
- [ ] 모든 사용자 생성 콘텐츠에 대한 출력 인코딩
- [ ] 모든 상태 변경 양식의 CSRF 토큰
- [ ] 보안 비밀번호 해싱(Argon2id)
- [ ] 세션 보안이 구성되었습니다.
- [ ] 파일 업로드 검증
- [ ] 보안 헤더가 설정되었습니다.
