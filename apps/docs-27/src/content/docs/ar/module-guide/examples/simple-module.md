---
title: "مثال وحدة بسيطة"
description: "وحدة عاملة كاملة مع جميع الملفات الضرورية"
dir: rtl
lang: ar
---

# مثال وحدة بسيطة - مدونة

وحدة عاملة بسيطة وكاملة توضح مفاهيم XOOPS الأساسية.
...
<h2>إدارة مشاركات المدونة</h2>
<a href="admin.php?op=create" class="btn btn-primary">مشاركة جديدة</a>

<table class="table">
    {foreach from=$posts item=post}
        <tr>
            <td>{$post->post_title}</td>
            <td>{if $post->post_published}منشور{else}مسودة{/if}</td>
            <td>
                <a href="admin.php?op=edit&id={$post->post_id}">تعديل</a>
                <a href="admin.php?op=delete&id={$post->post_id}">حذف</a>
            </td>
        </tr>
    {/foreach}
</table>
        ...
        <div class="posts-list">
            {foreach from=$posts item=post}
                <article class="post">
                    ...
                    <small>نُشرت في {$post->post_created|date_format:"%Y-%m-%d"}</small>
                    <p>{$post->post_content|truncate:200|escape}</p>
                    <a href="?op=view&id={$post->post_id}">اقرأ المزيد</a>
                </article>
            {/foreach}
        </div>
    {else}
        <p>لا توجد مشاركات متاحة.</p>
    {/if}>
</div>
```
...
<article class="blog-post">
    ...
    <small>نُشرت في {$post->post_created|date_format:"%Y-%m-%d %H:%M"}</small>
    
    <div class="post-content">
        {$post->post_content}
    </div>
    
    <a href="./">العودة إلى المدونة</a>
</article>
```
...
- عرض منشورات المدونة المنشورة مع الترقيم
- عرض منشورات فردية
- واجهة إدارية لإنشاء وتحرير وحذف المشاركات
- حالة المسودة/المنشورة
- الطوابع الزمنية

## اختبار

1. ثبت الوحدة عبر إدارة XOOPS
2. انقل إلى الواجهة الأمامية في `modules/blog/`
3. انقل إلى الإدارة في `modules/blog/admin.php`
4. أنشئ منشور اختبار وانشره

## الوثائق ذات الصلة

انظر أيضاً:
- ../Patterns/MVC-Pattern للأنماط
- ../Patterns/Repository-Pattern لوصول البيانات
- ../Best-Practices/Code-Organization للهيكل
