---
title: "Form Validierung"
---

## Übersicht

XOOPS bietet sowohl Client-seitige als auch Server-seitige Validierung für Form-Eingaben. Dieser Leitfaden behandelt Validierungstechniken, eingebaute Validatoren und benutzerdefinierte Validierungsimplementierung.

## Validierungs-Architektur

```mermaid
flowchart TB
    A[Form Submission] --> B{Client-Side Validation}
    B -->|Pass| C[Server Request]
    B -->|Fail| D[Show Client Errors]
    C --> E{Server-Side Validation}
    E -->|Pass| F[Process Data]
    E -->|Fail| G[Return Errors]
    G --> H[Display Server Errors]
```

## Server-Side Validierung

### Mit XoopsFormValidator

```php
use Xoops\Core\Form\Validator;

$validator = new Validator();

$validator->addRule('username', 'required', 'Username is required');
$validator->addRule('username', 'minLength:3', 'Username must be at least 3 characters');
$validator->addRule('username', 'maxLength:50', 'Username cannot exceed 50 characters');
$validator->addRule('email', 'email', 'Please enter a valid email address');
$validator->addRule('password', 'minLength:8', 'Password must be at least 8 characters');

if (!$validator->validate($_POST)) {
    $errors = $validator->getErrors();
    // Fehler bearbeiten
}
```

### Eingebaute Validierungs-Regeln

| Regel | Beschreibung | Beispiel |
|------|-------------|---------|
| `required` | Feld darf nicht leer sein | `required` |
| `email` | Gültiges E-Mail-Format | `email` |
| `url` | Gültiges URL-Format | `url` |
| `numeric` | Nur numerische Werte | `numeric` |
| `integer` | Integer Wert nur | `integer` |
| `minLength` | Minimale String-Länge | `minLength:3` |
| `maxLength` | Maximale String-Länge | `maxLength:100` |
| `min` | Minimaler numerischer Wert | `min:1` |
| `max` | Maximaler numerischer Wert | `max:100` |
| `regex` | Custom Regex Pattern | `regex:/^[a-z]+$/` |
| `in` | Wert in Liste | `in:draft,published,archived` |
| `date` | Gültiges Datums-Format | `date` |
| `alpha` | Nur Buchstaben | `alpha` |
| `alphanumeric` | Buchstaben und Zahlen | `alphanumeric` |

### Custom Validierungs-Regeln

```php
$validator->addCustomRule('unique_username', function($value) {
    $memberHandler = xoops_getHandler('member');
    $criteria = new \CriteriaCompo();
    $criteria->add(new \Criteria('uname', $value));
    return $memberHandler->getUserCount($criteria) === 0;
}, 'Username already exists');

$validator->addRule('username', 'unique_username');
```

## Request Validierung

### Eingaben bereinigen

```php
use Xoops\Core\Request;

// Bereinigte Werte abrufen
$username = Request::getString('username', '', 'POST');
$email = Request::getEmail('email', '', 'POST');
$age = Request::getInt('age', 0, 'POST');
$price = Request::getFloat('price', 0.0, 'POST');
$tags = Request::getArray('tags', [], 'POST');

// Mit Validierung
$username = Request::getString('username', '', 'POST', [
    'minLength' => 3,
    'maxLength' => 50
]);
```

### XSS Prevention

```php
use Xoops\Core\Text\Sanitizer;

$sanitizer = Sanitizer::getInstance();

// HTML-Inhalt bereinigen
$cleanContent = $sanitizer->sanitizeForDisplay($userContent);

// Alle HTML entfernen
$plainText = $sanitizer->stripHtml($userContent);

// Spezifische Tags zulassen
$content = $sanitizer->sanitizeForDisplay($userContent, [
    'allowedTags' => '<p><br><strong><em><a>'
]);
```

## Client-Side Validierung

### HTML5 Validierungs-Attribute

```php
// Erforderliches Feld
$element->setExtra('required');

// Pattern-Validierung
$element->setExtra('pattern="[a-zA-Z0-9]+" title="Alphanumeric only"');

// Längenbeschränkungen
$element->setExtra('minlength="3" maxlength="50"');

// Numerische Beschränkungen
$element->setExtra('min="1" max="100"');
```

### JavaScript Validierung

```javascript
document.getElementById('myForm').addEventListener('submit', function(e) {
    const username = document.getElementById('username').value;
    const errors = [];

    if (username.length < 3) {
        errors.push('Username must be at least 3 characters');
    }

    if (!/^[a-zA-Z0-9_]+$/.test(username)) {
        errors.push('Username can only contain letters, numbers, and underscores');
    }

    if (errors.length > 0) {
        e.preventDefault();
        displayErrors(errors);
    }
});
```

## CSRF Schutz

### Token-Generierung

```php
// Token in Form generieren
$form->addElement(new \XoopsFormHiddenToken());

// Dies fügt ein Hidden Field mit Security Token ein
```

### Token-Verifizierung

```php
use Xoops\Core\Security;

if (!Security::checkReferer()) {
    die('Invalid request origin');
}

if (!Security::checkToken()) {
    die('Invalid security token');
}
```

## File Upload Validierung

```php
use Xoops\Core\Uploader;

$uploader = new Uploader(
    uploadDir: XOOPS_UPLOAD_PATH . '/images/',
    allowedMimeTypes: ['image/jpeg', 'image/png', 'image/gif'],
    maxFileSize: 2 * 1024 * 1024, // 2MB
    maxWidth: 1920,
    maxHeight: 1080
);

if ($uploader->fetchMedia('image_upload')) {
    if ($uploader->upload()) {
        $savedFile = $uploader->getSavedFileName();
    } else {
        $errors[] = $uploader->getErrors();
    }
}
```

## Fehler-Anzeige

### Fehler sammeln

```php
$errors = [];

if (empty($username)) {
    $errors['username'] = 'Username is required';
}

if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    $errors['email'] = 'Invalid email format';
}

if (!empty($errors)) {
    // In Session für Anzeige nach Redirect speichern
    $_SESSION['form_errors'] = $errors;
    $_SESSION['form_data'] = $_POST;
    header('Location: ' . $_SERVER['HTTP_REFERER']);
    exit;
}
```

### Fehler anzeigen

```smarty
{if $errors}
<div class="alert alert-danger">
    <ul>
        {foreach $errors as $field => $message}
        <li>{$message}</li>
        {/foreach}
    </ul>
</div>
{/if}
```

## Best Practices

1. **Immer Server-seitig validieren** - Client-seitige Validierung kann umgangen werden
2. **Verwenden Sie parametrisierte Abfragen** - Verhindern Sie SQL Injection
3. **Bereinigen Sie Ausgabe** - Verhindern Sie XSS Angriffe
4. **Validieren Sie File Uploads** - Prüfen Sie MIME Types und Größen
5. **Verwenden Sie CSRF Tokens** - Verhindern Sie Cross-Site Request Forgery
6. **Rate Limit Submissions** - Verhindern Sie Missbrauch

## Verwandte Dokumentation

- Form Elements Reference
- Forms Overview
- Security Best Practices
