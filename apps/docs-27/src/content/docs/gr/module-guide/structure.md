...
Μια καλά οργανωμένη δομή μονάδας είναι θεμελιώδης για τη διατηρούμενη ανάπτυξη XOOPS. Αυτός ο οδηγός καλύπτει τόσο παλαιού τύπου όσο και σύγχρονες (PSR-4) διατάξεις μονάδων.

## Τυπική διάταξη ενότητας
...
| Κατάλογος | Σκοπός |
|-----------|---------|
| `admin/` | Διεπαφή διαχείρισης |
| `assets/` | CSS, JavaScript, εικόνες |
| `blocks/` | Λειτουργίες απόδοσης μπλοκ |
| `class/` | PHP τάξεις (κληρονομιά) |
| `config/` | Αρχεία διαμόρφωσης (σύγχρονα) |
| `include/` | Κοινόχρηστα αρχεία |
| `language/` | Αρχεία μετάφρασης |
| `migrations/` | Μετακινήσεις βάσεων δεδομένων |
| `sql/` | Αρχικό σχήμα βάσης δεδομένων |
| `src/` | PSR-4 πηγαίος κώδικας |
| `templates/` | Έξυπνα πρότυπα |
| `tests/` | Δοκιμαστικά αρχεία |

## Βέλτιστες πρακτικές

1. **Ξεχωριστές ανησυχίες** - Κρατήστε την επιχειρηματική λογική μακριά από τα πρότυπα
2. **Χρησιμοποιήστε Χώρους ονομάτων** - Οργάνωσε τον κώδικα με τον κατάλληλο χώρο ονομάτων
3. **Ακολουθήστε το PSR-4** - Χρησιμοποιήστε τυπικές συμβάσεις αυτόματης φόρτωσης
4. **Externalize Config** - Διατηρήστε τη διαμόρφωση χωριστά από τον κώδικα
5. **Δομή εγγράφου** - Συμπεριλάβετε το README επεξηγητικό οργανισμό

## Σχετική τεκμηρίωση

- Module-Development - Πλήρης οδηγός ανάπτυξης
- Best-Practices/Code-Organization - Μοτίβα οργάνωσης κωδικών
- Module Manifest - Διαμόρφωση Manifest
- Database/Database-Schema - Σχεδιασμός βάσης δεδομένων
