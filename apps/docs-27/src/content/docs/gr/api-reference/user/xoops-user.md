---
title: "Αναφορά XoopsUser API"
description: "Πλήρης αναφορά API για τάξεις XoopsUser και διαχείρισης χρηστών"
---

> Ολοκληρώστε την τεκμηρίωση API για το σύστημα χρήστη XOOPS.
...
|----------|------|-------------|
| `uid` | int | Αναγνωριστικό χρήστη (πρωτεύον κλειδί) |
| `uname` | χορδή | Όνομα χρήστη |
| `name` | χορδή | Πραγματικό όνομα |
| `email` | χορδή | Διεύθυνση ηλεκτρονικού ταχυδρομείου |
| `pass` | χορδή | Κατακερματισμός κωδικού πρόσβασης |
| `url` | χορδή | Ιστοσελίδα URL |
| `user_avatar` | χορδή | Όνομα αρχείου Avatar |
| `user_regdate` | int | Χρονική σήμανση εγγραφής |
| `user_from` | χορδή | Τοποθεσία |
| `user_sig` | χορδή | Υπογραφή |
| `user_occ` | χορδή | Επάγγελμα |
| `user_intrest` | χορδή | Ενδιαφέροντα |
| `bio` | χορδή | Βιογραφία |
| `posts` | int | Καταμέτρηση αναρτήσεων |
| `rank` | int | Κατάταξη χρήστη |
| `level` | int | Επίπεδο χρήστη (0=ανενεργός, 1=ενεργός) |
| `theme` | χορδή | Προτιμώμενο θέμα |
| `timezone` | πλωτήρας | Μετατόπιση ζώνης ώρας |
| `last_login` | int | Χρονική σήμανση τελευταίας σύνδεσης |
...
| Αναγνωριστικό ομάδας | Όνομα | Περιγραφή |
|----------|------|-------------|
| 1 | Webmasters | Πλήρης πρόσβαση διαχειριστή |
| 2 | Εγγεγραμμένοι Χρήστες | Τυπικοί εγγεγραμμένοι χρήστες |
| 3 | Ανώνυμος | Μη συνδεδεμένοι επισκέπτες |
