...
> 使用 Smarty、CSS 和 HTML 覆盖创建和自定义 Publisher 模板的指南。
...
在 `/modules/publisher/XOOPS_version.php` 或管理设置中：
