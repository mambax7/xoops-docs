...
区块是显示在主题侧边栏和内容区域中的可重用内容小部件。本指南涵盖创建、配置和自定义 XOOPS 区块。

## 区块结构

### XOOPS_version 中的区块定义。php
...
| `description` |描述语言常数 |
| `show_func` |渲染区块内容的函数 |
| `edit_func` |渲染区块选项表单的函数 |
| `template`| Smarty 模板文件 |
| `options` |管道-separated默认选项|
...
2. **验证选项** - 始终验证区块选项
3. **转义输出** - 清理所有用户内容
4. **使用 Criteria** - 使用 Criteria 类构建查询
5. **限制查询** - 设置合理的性能限制
6. **响应式模板** - 确保区块在移动设备上工作

## 相关文档

- 模区块-Development - 模区块创建指南
- ../02-Core-Concepts/Templates/Smarty-Templating - 模板语法
