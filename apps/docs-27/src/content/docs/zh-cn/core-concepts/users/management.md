...
XOOPS用户管理系统提供了一个完整的框架来处理用户注册、身份验证、配置文件管理和用户首选项。本文档涵盖了用户对象结构、注册流程和实际实现示例。

## 用户对象结构

XOOPS中的核心用户对象是`XOOPSUser`类，它封装了所有用户数据和方法。
...
- 始终使用 `password_hash()` 和 `PASSWORD_BCRYPT` 算法
- bcrypt 使用成本参数 12
- 切勿存储纯文本密码
- 实施密码过期策略
- 需要更改受感染帐户的密码
