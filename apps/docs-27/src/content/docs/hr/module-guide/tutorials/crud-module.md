---
title: "Izrada CRUD modula"
description: "Cjeloviti vodič za izgradnju modula CRUD s operacijama baze podataka, obrascima i sučeljem admin"
---
# Vodič za izradu CRUD modula

Ovaj vodič vas vodi kroz izgradnju potpunog modula CRUD (Stvaranje, čitanje, ažuriranje, brisanje) za XOOPS. Napravit ćemo modul "Bilješke" koji korisnicima omogućuje upravljanje osobnim bilješkama.

## Preduvjeti

- Dovršen vodič Hello-World-Module
- Razumijevanje PHP OOP koncepata
- Osnovno znanje SQL

## Pregled modula

**Značajke modula Notes:**
- Stvaranje, pregled, uređivanje i brisanje bilješki
- Administratorsko sučelje za upravljanje
- Bilješke specifične za korisnika
- Organizacija kategorija
- Funkcionalnost pretraživanja

## Korak 1: Struktura imenika

Napravite sljedeću strukturu u `/modules/notes/`:
...
## Korak 2: Shema baze podataka

Kreiraj `sql/mysql.sql`:
...
## Korak 3: Definicija modula

Kreiraj `xoops_version.php`:
...
## Korak 4: Klase entiteta

### Bilješka Entitet

Kreiraj `class/Note.php`:
...
### Rukovatelj bilješkama

Kreiraj `class/NoteHandler.php`:
...
### Rukovatelj kategorijama

Kreirajte `class/CategoryHandler.php`:
...
## Korak 5: Uobičajena uključena datoteka

Kreirajte `include/common.php`:
...
### Uredi stranicu (Stvori/ažuriraj)
...
Kreirajte `templates/notes_index.tpl`:
...
Kreiraj `templates/notes_view.tpl`:
...
Kreiraj `templates/notes_edit.tpl`:
...
## Korak 9: Administratorsko sučelje

### Administratorski izbornik

Kreirajte `admin/menu.php`:
...
### Upravljanje bilješkama administratora

Kreirajte `admin/notes.php`:
...
čestitamo! Napravili ste potpuni modul CRUD. Ključni pojmovi obuhvaćeni:

1. **Dizajn baze podataka** - Tablice s relacijama
2. **Klase entiteta** - XoopsObject s upisanim svojstvima
3. **Klase rukovatelja** - XoopsPersistableObjectHandler s prilagođenim metodama
4. **Frontend stranice** - Funkcionalnost popisa, pregleda i uređivanja
5. **Rukovanje obrascima** - CSRF zaštita i provjera valjanosti
6. **Administratorsko sučelje** - Zasloni za upravljanje
7. **predlošci** - Smarty templates s logikom

## Sljedeći koraci

- Dodajte više naprednih značajki (komentari, ocjene, dijeljenje)
- Implementirajte ../Patterns/Repository-Pattern za čistiji pristup podacima
- Primijenite ../Patterns/MVC-Pattern za bolju organizaciju koda
- Dodajte ../Best-Practices/Testing with PHPUnit

Vidi također: ../Razvoj modula | ../Uzorci/MVC-Uzorak | ../Patterns/Repository-Pattern
