---
title: "xoops_version.php - Manifest modula"
---
## Pregled

`xoops_version.php` datoteka je srce svakog XOOPS modula. Definira metapodatke modula, tablice baze podataka, templates, blokove, opcije konfiguracije i instalacijske kuke.
...
| Ključ | Upišite | Opis |
|-----|------|-------------|
| `name` | niz | Ime za prikaz (koristite konstantu language) |
| `version` | niz | Semantička verzija (MAJOR.MINOR.PATCH) |
| `description` | niz | Opis modula |
| `author` | niz | Ime primarnog autora |
| `credits` | niz | Dodatni suradnici |
| `license` | niz | Naziv licence |
...
| vrsta obrasca | vrsta vrijednosti | Opis |
|----------|-----------|-------------|
| `textbox` | `text`/`int` | Jednoredni unos |
| `textarea` | `text` | Višelinijski unos |
| `yesno` | `int` | Da/Ne radio |
| `select` | `text` | Padajući odabir |
| `select_multi` | `array` | Višestruki odabir |
| `group` | `int` | Izbornik grupe |
| `group_multi` | `array` | Izbornik više grupa |
| `user` | `int` | Birač korisnika |
| `color` | `text` | Birač boja |
| `hidden` | `text` | Skriveno polje |
...
- Razvoj modula - Potpuni vodič za module
- Struktura modula - Struktura imenika
- Block-Development - Stvaranje blokova
- Operacije baze podataka - Postavljanje baze podataka
