---
title: "Pomoćnik za dozvole"
description: "Upravljanje dopuštenjima grupe XOOPS pomoću pomoćnika za dopuštenja XMF"
---
XOOPS ima moćan i fleksibilan sustav dopuštenja temeljen na članstvu u grupi korisnika. XMF Permission Helper pojednostavljuje rad s ovim dopuštenjima, smanjujući složene provjere dopuštenja na pozive jedne metode.

## Pregled

Sustav dozvola XOOPS povezuje grupe sa:
- ID modula
- Naziv dopuštenja
- ID artikla

Provjera dopuštenja tradicionalno zahtijeva pronalaženje korisničkih grupa, traženje ID-ova modula i postavljanje upita u tablicama dopuštenja. XMF Permission Helper sve to automatski obrađuje.

## Početak

### Stvaranje pomoćnika za dozvole
...
Pomoćnik automatski koristi trenutne korisničke grupe i navedeni ID modula.

## Provjera dopuštenja

### Ima li korisnik dopuštenje?

Provjerite ima li trenutačni korisnik određenu dozvolu za stavku:
...
### Provjerite s preusmjeravanjem

Automatski preusmjeri korisnike koji nemaju dozvolu:
...
### Administratorsko nadjačavanje

Prema zadanim postavkama, korisnici admin uvijek imaju dozvolu. Za provjeru čak i za admins:
...
### Dobijte dopuštene ID-ove stavki

Dohvatite sve ID-ove stavki za koje određene grupe imaju dopuštenje:
...
### Nabavite grupe za stavku

Pronađite koje grupe imaju određenu dozvolu:
...
Dodijeli dopuštenje određenim grupama:
...
Uklonite sva dopuštenja za stavku (obično kada brišete stavku):
...
## Integracija obrazaca

### Dodavanje odabira dopuštenja obrascima

Pomoćnik može stvoriti element obrasca za odabir grupa:
...
### Opcije elemenata obrasca

Potpuni potpis metode:
...
Pomoćnik generira dosljedna imena polja:
...
## Kompletan primjer: Stavke zaštićene dopuštenjem

### Stvaranje stavke s dopuštenjima
...
### Gledanje s provjerom dopuštenja
...
### Brisanje uz čišćenje dopuštenja
...
| Metoda | Opis |
|--------|-------------|
| `checkPermission($name, $itemId, $trueIfAdmin)` | Provjerite ima li korisnik dopuštenje |
| `checkPermissionRedirect($name, $itemId, $url, $time, $message, $trueIfAdmin)` | Provjerite i preusmjerite ako je odbijen |
| `getItemIds($name, $groupIds)` | Dohvati ID-ove stavki koje grupe mogu pristupiti |
| `getGroupsForItem($name, $itemId)` | Dobiti grupe s dopuštenjem |
| `savePermissionForItem($name, $itemId, $groups)` | Spremi dopuštenja |
| `deletePermissionForItem($name, $itemId)` | Brisanje dopuštenja |
| `getGroupSelectFormForItem(...)` | Kreiraj element odabira obrasca |
| `defaultFieldName($name, $itemId)` | Dohvati zadani naziv polja obrasca |

## Vidi također

- ../Basics/XMF-Module-Helper - Dokumentacija modula Helper
- Module-Admin-Pages - Kreiranje administratorskog sučelja
- ../Basics/Getting-Started-with-XMF - XMF osnove

---

#xmf #dozvole #sigurnost #grupe #forme
