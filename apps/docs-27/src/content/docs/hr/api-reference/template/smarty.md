---
title: "Smarty predložak API Referenca"
description: "Kompletna API referenca za Smarty predložak u XOOPS"
---
> Kompletna API dokumentacija za Smarty predložak u XOOPS.

---

## Arhitektura motora predložaka
...
|--------|------------|-------------|
| `assign` | `string $name, mixed $value` | Dodijelite varijablu predlošku |
| `assignByRef` | `string $name, mixed &$value` | Dodijeli referencom |
| `append` | `string $name, mixed $value, bool $merge = false` | Dodavanje varijabli polja |
| `display` | `string $template` | predložak za prikaz i izlaz |
| `fetch` | `string $template` | Renderiraj i vrati predložak |
| `clearAssign` | `string $name` | Obriši dodijeljenu varijablu |
| `clearAllAssign` | - | Obriši sve varijable |
| `getTemplateVars` | `string $name = null` | Dobiti dodijeljene varijable |
| `templateExists` | `string $template` | Provjerite postoji li predložak |
| `isCached` | `string $template` | Provjerite je li predložak cached |
| `clearCache` | `string $template = null` | Čisti predložak cache |
...
## XOOPS-specifične funkcije

### Blokiraj iscrtavanje
...
1. **Omogućite predmemoriranje predložaka** u proizvodnji
2. **Dodijelite samo potrebne varijable** - nemojte prosljeđivati cijele objekte
3. **Umjereno koristite modifikatore** - unaprijed formatirajte u PHP kada je to moguće
4. **Izbjegavajte ugniježđene petlje** - restrukturirajte podatke u PHP
5. **Predmemorirajte skupe blokove** - koristite predmemoriju blokova za složene upite
...
#xoops #api #smarty #templates #referenca
