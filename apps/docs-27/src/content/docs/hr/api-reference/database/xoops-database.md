---
title: "Xoops klasa baze podataka"
description: "Sloj apstrakcije baze podataka koji omogućuje upravljanje vezom, izvršavanje upita i rukovanje rezultatima"
---
`XoopsDatabase` class pruža sloj apstrakcije baze podataka za XOOPS, upravljajući upravljanjem vezom, izvršavanjem upita, obradom rezultata i rukovanjem greškama. Podržava više upravljačkih programa baze podataka kroz arhitekturu upravljačkog programa.

## Pregled razreda

```php
namespace Xoops\Database;

abstract class XoopsDatabase
{
    protected $conn;
    protected $prefix;
    protected $logger;

    abstract public function connect(bool $selectdb = true): bool;
    abstract public function query(string $sql, int $limit = 0, int $start = 0);
    abstract public function fetchArray($result): ?array;
    abstract public function fetchObject($result): ?object;
    abstract public function getRowsNum($result): int;
    abstract public function getAffectedRows(): int;
    abstract public function getInsertId(): int;
    abstract public function escape(string $string): string;
}
```

## Hijerarhija klasa

```
XoopsDatabase (Abstract Base)
├── XoopsMySQLDatabase (MySQL Extension)
│   └── XoopsMySQLDatabaseProxy (Security Proxy)
└── XoopsMySQLiDatabase (MySQLi Extension)
    └── XoopsMySQLiDatabaseProxy (Security Proxy)

XoopsDatabaseFactory
└── Creates appropriate driver instances
```

## Dobivanje instance baze podataka

### Korištenje tvornice

```php
// Recommended: Use the factory
$db = XoopsDatabaseFactory::getDatabaseConnection();
```

### Korištenje getInstance

```php
// Alternative: Direct singleton access
$db = XoopsDatabase::getInstance();
```

### Globalna varijabla

```php
// Legacy: Use global variable
global $xoopsDB;
```

## Temeljne metode

### poveži se

Uspostavlja vezu s bazom podataka.

```php
abstract public function connect(bool $selectdb = true): bool
```

**Parametri:**

| Parametar | Upišite | Opis |
|-----------|------|-------------|
| `$selectdb` | bool | Treba li odabrati bazu podataka |

**Vraća:** `bool` - Istina pri uspješnom povezivanju

**Primjer:**
```php
$db = XoopsDatabaseFactory::getDatabaseConnection();
if ($db->connect()) {
    echo "Connected successfully";
}
```

---

### upit

Izvršava SQL upit.

```php
abstract public function query(
    string $sql,
    int $limit = 0,
    int $start = 0
): mixed
```

**Parametri:**

| Parametar | Upišite | Opis |
|-----------|------|-------------|
| `$sql` | niz | SQL niz upita |
| `$limit` | int | Maksimalan broj redaka za vraćanje (0 = nema ograničenja) |
| `$start` | int | Početni pomak |

**Vraća:** `resource|bool` - Resurs rezultata ili false u slučaju neuspjeha

**Primjer:**
```php
$db = XoopsDatabaseFactory::getDatabaseConnection();

// Simple query
$sql = "SELECT * FROM " . $db->prefix('users') . " WHERE uid > 0";
$result = $db->query($sql);

// Query with limit
$sql = "SELECT * FROM " . $db->prefix('users');
$result = $db->query($sql, 10, 0); // First 10 rows

// Query with offset
$result = $db->query($sql, 10, 20); // 10 rows starting at row 20
```

---

### upitF

Izvršava upit prisiljavajući operaciju (zaobilazi sigurnosne provjere).

```php
public function queryF(string $sql, int $limit = 0, int $start = 0): mixed
```

**Slučajevi upotrebe:**
- Operacije INSERT, UPDATE, DELETE
- Kada trebate zaobići ograničenja samo za čitanje

**Primjer:**
```php
$sql = sprintf(
    "UPDATE %s SET views = views + 1 WHERE article_id = %d",
    $db->prefix('articles'),
    $articleId
);
$db->queryF($sql);
```

---

### prefiks

Dodaje ispred prefiksa tablice baze podataka.

```php
public function prefix(string $table = ''): string
```

**Parametri:**

| Parametar | Upišite | Opis |
|-----------|------|-------------|
| `$table` | niz | Naziv tablice bez prefiksa |

**Vraća:** `string` - Naziv tablice s prefiksom

**Primjer:**
```php
$db = XoopsDatabaseFactory::getDatabaseConnection();

echo $db->prefix('users');       // "xoops_users" (if prefix is "xoops_")
echo $db->prefix('modules');     // "xoops_modules"
echo $db->prefix();              // "xoops_" (just the prefix)
```

---

### dohvati niz

Dohvaća red rezultata kao asocijativno polje.

```php
abstract public function fetchArray($result): ?array
```

**Parametri:**

| Parametar | Upišite | Opis |
|-----------|------|-------------|
| `$result` | resurs | Resurs rezultata upita |

**Vraća:** `array|null` - Asocijativni niz ili null ako nema više redaka

**Primjer:**
```php
$sql = "SELECT * FROM " . $db->prefix('users') . " WHERE level > 0";
$result = $db->query($sql);

while ($row = $db->fetchArray($result)) {
    echo "User: " . $row['uname'] . "\n";
    echo "Email: " . $row['email'] . "\n";
}
```

---

### dohvati Objekt

Dohvaća red rezultata kao objekt.

```php
abstract public function fetchObject($result): ?object
```

**Parametri:**

| Parametar | Upišite | Opis |
|-----------|------|-------------|
| `$result` | resurs | Resurs rezultata upita |

**Vraća:** `object|null` - Objekt sa svojstvima za svaki stupac

**Primjer:**
```php
$sql = "SELECT * FROM " . $db->prefix('users') . " WHERE uid = 1";
$result = $db->query($sql);

if ($user = $db->fetchObject($result)) {
    echo "Username: " . $user->uname;
    echo "Email: " . $user->email;
}
```

---

### dohvati red

Dohvaća red rezultata kao numeričko polje.

```php
abstract public function fetchRow($result): ?array
```

**Primjer:**
```php
$sql = "SELECT uname, email FROM " . $db->prefix('users');
$result = $db->query($sql);

while ($row = $db->fetchRow($result)) {
    echo "Username: " . $row[0] . ", Email: " . $row[1];
}
```

---

### dohvati Oboje

Dohvaća red rezultata kao asocijativno i numeričko polje.

```php
abstract public function fetchBoth($result): ?array
```

**Primjer:**
```php
$result = $db->query($sql);
$row = $db->fetchBoth($result);
echo $row['uname'];  // By name
echo $row[0];        // By index
```

---

### getRowsNum

Dobiva broj redaka u skupu rezultata.

```php
abstract public function getRowsNum($result): int
```

**Parametri:**

| Parametar | Upišite | Opis |
|-----------|------|-------------|
| `$result` | resurs | Resurs rezultata upita |

**Povratak:** `int` - Broj redaka

**Primjer:**
```php
$sql = "SELECT * FROM " . $db->prefix('users') . " WHERE level > 0";
$result = $db->query($sql);
$count = $db->getRowsNum($result);
echo "Found $count active users";
```

---

### getAffectedRows

Dohvaća broj zahvaćenih redaka iz zadnjeg upita.

```php
abstract public function getAffectedRows(): int
```

**Vraća:** `int` - Broj zahvaćenih redaka

**Primjer:**
```php
$sql = "UPDATE " . $db->prefix('users') . " SET last_login = " . time() . " WHERE uid = 1";
$db->queryF($sql);
$affected = $db->getAffectedRows();
echo "Updated $affected rows";
```

---

### getInsertId

Dobiva automatski generirani ID iz posljednjeg INSERT-a.

```php
abstract public function getInsertId(): int
```
**Povratak:** `int` - ID posljednjeg umetanja

**Primjer:**
```php
$sql = sprintf(
    "INSERT INTO %s (title, content) VALUES (%s, %s)",
    $db->prefix('articles'),
    $db->quoteString($title),
    $db->quoteString($content)
);
$db->queryF($sql);
$newId = $db->getInsertId();
echo "Created article with ID: $newId";
```

---

### bijeg

Izbjegava niz za sigurnu upotrebu u SQL upitima.

```php
abstract public function escape(string $string): string
```

**Parametri:**

| Parametar | Upišite | Opis |
|-----------|------|-------------|
| `$string` | niz | Niz za izlaz |

**Vraća:** `string` - Escaped string (bez navodnika)

**Primjer:**
```php
$unsafeInput = "O'Reilly";
$safe = $db->escape($unsafeInput);  // "O\'Reilly"

$sql = "SELECT * FROM " . $db->prefix('users') . " WHERE uname = '" . $safe . "'";
```

---

### quoteString

Izbjegava i stavlja u navodnike niz za SQL.

```php
public function quoteString(string $string): string
```

**Parametri:**

| Parametar | Upišite | Opis |
|-----------|------|-------------|
| `$string` | niz | String za citiranje |

**Vraća:** `string` - Escaped i citirani niz

**Primjer:**
```php
$name = "John O'Connor";
$quoted = $db->quoteString($name);  // "'John O\'Connor'"

$sql = "INSERT INTO users (name) VALUES (" . $quoted . ")";
```

---

### freeRecordSet

Oslobađa memoriju povezanu s rezultatom.

```php
abstract public function freeRecordSet($result): void
```

**Primjer:**
```php
$result = $db->query($sql);
// Process results...
$db->freeRecordSet($result);  // Free memory
```

---

## Rješavanje grešaka

### greška

Dobiva posljednju poruku o pogrešci.

```php
abstract public function error(): string
```

**Primjer:**
```php
$result = $db->query($sql);
if (!$result) {
    echo "Database error: " . $db->error();
}
```

---

### pogreška

Dobiva posljednji broj pogreške.

```php
abstract public function errno(): int
```

**Primjer:**
```php
$result = $db->query($sql);
if (!$result) {
    echo "Error #" . $db->errno() . ": " . $db->error();
}
```

---

## Pripremljeni iskazi (MySQLi)

MySQLi upravljački program podržava pripremljene izjave za poboljšanu sigurnost.

### pripremiti

Stvara pripremljenu izjavu.

```php
public function prepare(string $sql): mysqli_stmt|false
```

**Primjer:**
```php
$db = XoopsDatabaseFactory::getDatabaseConnection();

$sql = "SELECT * FROM " . $db->prefix('users') . " WHERE uid = ?";
$stmt = $db->prepare($sql);

$stmt->bind_param('i', $userId);
$userId = 5;
$stmt->execute();
$result = $stmt->get_result();

while ($row = $result->fetch_assoc()) {
    echo $row['uname'];
}
$stmt->close();
```

### Pripremljeni iskaz s više parametara

```php
$sql = "INSERT INTO " . $db->prefix('articles') . " (title, content, author_id) VALUES (?, ?, ?)";
$stmt = $db->prepare($sql);

$stmt->bind_param('ssi', $title, $content, $authorId);

$title = "My Article";
$content = "Article content here";
$authorId = 1;

if ($stmt->execute()) {
    echo "Article created with ID: " . $stmt->insert_id;
}

$stmt->close();
```

---

## Transakcijska podrška

### započeti transakciju

Započinje transakciju.

```php
public function beginTransaction(): bool
```

### počiniti

Obvezuje trenutnu transakciju.

```php
public function commit(): bool
```

### vraćanje

Vraća trenutnu transakciju.

```php
public function rollback(): bool
```

**Primjer:**
```php
$db = XoopsDatabaseFactory::getDatabaseConnection();

try {
    $db->beginTransaction();

    // Multiple operations
    $sql1 = "UPDATE " . $db->prefix('accounts') . " SET balance = balance - 100 WHERE id = 1";
    $db->queryF($sql1);

    $sql2 = "UPDATE " . $db->prefix('accounts') . " SET balance = balance + 100 WHERE id = 2";
    $db->queryF($sql2);

    // Check for errors
    if ($db->errno()) {
        throw new Exception($db->error());
    }

    $db->commit();
    echo "Transaction completed";

} catch (Exception $e) {
    $db->rollback();
    echo "Transaction failed: " . $e->getMessage();
}
```

---

## Potpuni primjeri upotrebe

### Osnovne operacije CRUD

```php
$db = XoopsDatabaseFactory::getDatabaseConnection();

// CREATE
$sql = sprintf(
    "INSERT INTO %s (title, content, created) VALUES (%s, %s, %d)",
    $db->prefix('articles'),
    $db->quoteString('New Article'),
    $db->quoteString('Article content'),
    time()
);
$db->queryF($sql);
$articleId = $db->getInsertId();

// READ
$sql = "SELECT * FROM " . $db->prefix('articles') . " WHERE id = " . (int)$articleId;
$result = $db->query($sql);
$article = $db->fetchArray($result);

// UPDATE
$sql = sprintf(
    "UPDATE %s SET title = %s, updated = %d WHERE id = %d",
    $db->prefix('articles'),
    $db->quoteString('Updated Title'),
    time(),
    $articleId
);
$db->queryF($sql);

// DELETE
$sql = "DELETE FROM " . $db->prefix('articles') . " WHERE id = " . (int)$articleId;
$db->queryF($sql);
```

### Upit za paginaciju

```php
function getArticles(int $page = 1, int $perPage = 10): array
{
    $db = XoopsDatabaseFactory::getDatabaseConnection();
    $start = ($page - 1) * $perPage;

    // Get total count
    $sql = "SELECT COUNT(*) as total FROM " . $db->prefix('articles') . " WHERE published = 1";
    $result = $db->query($sql);
    $row = $db->fetchArray($result);
    $total = $row['total'];

    // Get page of results
    $sql = "SELECT * FROM " . $db->prefix('articles') .
           " WHERE published = 1 ORDER BY created DESC";
    $result = $db->query($sql, $perPage, $start);

    $articles = [];
    while ($row = $db->fetchArray($result)) {
        $articles[] = $row;
    }

    return [
        'articles' => $articles,
        'total' => $total,
        'pages' => ceil($total / $perPage),
        'current' => $page
    ];
}
```

### Upit za pretraživanje sa LIKE

```php
function searchArticles(string $keyword): array
{
    $db = XoopsDatabaseFactory::getDatabaseConnection();

    $keyword = $db->escape($keyword);
    $sql = "SELECT * FROM " . $db->prefix('articles') .
           " WHERE title LIKE '%" . $keyword . "%'" .
           " OR content LIKE '%" . $keyword . "%'" .
           " ORDER BY created DESC";

    $result = $db->query($sql, 50);  // Limit to 50 results

    $articles = [];
    while ($row = $db->fetchArray($result)) {
        $articles[] = $row;
    }

    return $articles;
}
```

### Pridružite se upitu

```php
function getArticlesWithAuthors(): array
{
    $db = XoopsDatabaseFactory::getDatabaseConnection();

    $sql = "SELECT a.*, u.uname as author_name, u.email as author_email
            FROM " . $db->prefix('articles') . " a
            LEFT JOIN " . $db->prefix('users') . " u ON a.author_id = u.uid
            WHERE a.published = 1
            ORDER BY a.created DESC";

    $result = $db->query($sql, 20);

    $articles = [];
    while ($row = $db->fetchArray($result)) {
        $articles[] = $row;
    }

    return $articles;
}
```

---

## SqlUtility klasa

Pomoćnik class za operacije datoteka SQL.

### splitMySqlFile

Rastavlja SQL datoteku u pojedinačne upite.

```php
public static function splitMySqlFile(string $content): array
```

**Primjer:**
```php
$sqlContent = file_get_contents('install.sql');
$queries = SqlUtility::splitMySqlFile($sqlContent);

foreach ($queries as $query) {
    $db->queryF($query);
    if ($db->errno()) {
        echo "Error executing: " . $query . "\n";
        echo "Error: " . $db->error() . "\n";
    }
}
```

### prefixQuery

Zamjenjuje rezervirana mjesta u tablici nazivima tablica s prefiksom.

```php
public static function prefixQuery(string $sql, string $prefix): string
```

**Primjer:**
```php
$sql = "CREATE TABLE {PREFIX}_articles (id INT PRIMARY KEY)";
$prefixedSql = SqlUtility::prefixQuery($sql, $db->prefix());
// "CREATE TABLE xoops_articles (id INT PRIMARY KEY)"
```

---

## Najbolji primjeri iz prakse

### Sigurnost

1. **Uvijek izbjegavaj korisnički unos**:
```php
$safe = $db->escape($_POST['input']);
```

2. **Koristite pripremljene izjave kada su dostupne**:
```php
$stmt = $db->prepare("SELECT * FROM users WHERE id = ?");
$stmt->bind_param('i', $id);
```

3. **Koristite quoteString za vrijednosti**:
```php
$sql = "INSERT INTO table (name) VALUES (" . $db->quoteString($name) . ")";
```

### Izvedba

1. **Uvijek koristite LIMIT za velike tablice**:
```php
$result = $db->query($sql, 100);  // Limit results
```

2. **Setovi besplatnih rezultata po završetku**:
```php
$db->freeRecordSet($result);
```

3. **Koristite odgovarajuće indekse** u definicijama tablica

4. **Preferirajte rukovatelje umjesto sirovog SQL** kada je to moguće

### Rješavanje grešaka

1. **Uvijek provjerite pogreške**:
```php
$result = $db->query($sql);
if (!$result) {
    trigger_error($db->error(), E_USER_WARNING);
}
```

2. **Koristite transakcije za više povezanih operacija**:
```php
$db->beginTransaction();
// ... operations ...
$db->commit();  // or $db->rollback();
```

## Povezana dokumentacija

- Kriteriji - Sustav kriterija upita
- QueryBuilder - Tečna izgradnja upita
- ../Core/XoopsObjectHandler - Postojanost objekta

---

*Vidi također: [XOOPS izvorni kod](https://github.com/XOOPS/XoopsCore27/tree/master/htdocs/class/database)*
