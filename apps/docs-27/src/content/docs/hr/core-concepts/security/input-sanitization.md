---
title: "Dezinfekcija ulaza"
description: "Korištenje MyTextSanitizera i tehnika provjere valjanosti u XOOPS"
---
Nikad ne vjerujte korisničkom unosu. Uvijek provjerite i očistite sve ulazne podatke prije upotrebe. XOOPS pruža `MyTextSanitizer` class za dezinfekciju unosa teksta i razne pomoćne funkcije za provjeru valjanosti.
...
**Nikada ne vjerujte korisničkom unosu.** Svi podaci iz vanjskih izvora moraju biti:

1. **Provjereno**: Provjerite odgovara li očekivanom formatu i vrsti
2. **Dezinficirano**: Uklonite ili izbjegnite potencijalno opasne likove
3. **Escaped**: Prilikom ispisa, escape za određeni kontekst (HTML, JavaScript, SQL)

## MyTextSanitizer klasa

XOOPS pruža `MyTextSanitizer` class (obično poznat kao `$myts`) za čišćenje teksta.
...
### Obrada sadržaja tekstualnog područja

Metoda `displayTarea()` pruža sveobuhvatnu obradu textarea:
...
### Provjera cjelobrojnih vrijednosti
...
### Provjera valjanosti adresa e-pošte
...
### Provjera valjanosti naziva datoteka
...
Kada koristite XMF, Zahtjev class pruža čistije rukovanje unosom:
...
## Stvaranje klase provjere valjanosti

Za složene obrasce kreirajte namjensku provjeru class:
...
## Čišćenje za pohranu baze podataka

Prilikom pohranjivanja podataka u bazu podataka:
...
## Dezinfekcija zaslona

Različiti konteksti zahtijevaju različito bježanje:
...
**Problem**: podaci se kodiraju više puta
...
### Nedosljedno kodiranje

**Problem**: neki izlazi su kodirani, neki nisu

**Rješenje**: Uvijek koristite dosljedan pristup, po mogućnosti kodiranje na izlazu:
...
**Problem**: samo dezinfekcija bez provjere

**Rješenje**: Uvijek prvo provjerite, a zatim dezinficirajte:
...
1. **Koristite MyTextSanitizer** za obradu tekstualnog sadržaja
2. **Koristite filter_var()** za provjeru valjanosti određenog formata
3. **Koristite pretvaranje tipa** za numeričke vrijednosti
4. **Popis dopuštenih vrijednosti** za odabrane unose
5. **Potvrdite prije dezinfekcije**
6. **Escape na izlazu**, ne na ulazu
7. **Koristite pripremljene izjave** za upite baze podataka
8. **Stvorite potvrdu classes** za složene obrasce
9. **Nikada ne vjerujte provjeri valjanosti na strani klijenta** - uvijek provjeravajte valjanost na strani poslužitelja

---

#sigurnost #sanitizacija #validacija #xoops #MyTextSanitizer #input-handling
