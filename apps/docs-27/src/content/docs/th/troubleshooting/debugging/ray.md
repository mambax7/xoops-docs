---
title: "การใช้เรย์ดีบักเกอร์"
description: "การดีบักแอปพลิเคชัน XOOPS ด้วย Ray debugger"
---
# การใช้ Ray Debugger สำหรับ XOOPS

> การแก้ไขจุดบกพร่องสมัยใหม่ด้วย Ray: ตรวจสอบตัวแปร บันทึกข้อความ ติดตามการสืบค้น SQL และประสิทธิภาพของโปรไฟล์ในแอปพลิเคชัน XOOPS ของคุณ

---

## เรย์คืออะไร?

Ray เป็นเครื่องมือแก้ไขจุดบกพร่องขนาดเล็กที่ช่วยให้คุณตรวจสอบสถานะแอปพลิเคชันโดยไม่ต้องหยุดการดำเนินการหรือใช้เบรกพอยต์ เหมาะอย่างยิ่งสำหรับการพัฒนา XOOPS
...
### ขั้นตอนที่ 1: ติดตั้ง Ray Package
```bash
cd /path/to/xoops
...
```
### ขั้นตอนที่ 2: ดาวน์โหลดแอป Ray

ดาวน์โหลดจาก [ray.so](https://ray.so):
- แมค: Ray.app
- วินโดวส์: Ray.exe
- Linux: เรย์ (AppImage)
...
Ray ใช้พอร์ต 23517 เป็นค่าเริ่มต้น:
```bash
# UFW
...
?>
```
### 4. การจัดกลุ่มบันทึกที่เกี่ยวข้อง
```php
<?php
$ray = ray();
...
## ฟังก์ชั่นเรย์แบบกำหนดเอง

### สร้างฟังก์ชั่นตัวช่วย
```php
<?php
...
### ถาม: เรย์ไม่ได้รับข้อความ

**ก:**
1. ตรวจสอบว่าแอป Ray กำลังทำงานอยู่
2. ตรวจสอบไฟร์วอลล์อนุญาตพอร์ต 23517
3. ตรวจสอบว่าติดตั้ง Ray แล้ว:
```bash
composer require spatie/ray
```
### ถาม: ไม่เห็นข้อความค้นหา SQL
...
?>
```
### ถาม: ผลกระทบด้านประสิทธิภาพของ Ray

**A:** เรย์มีค่าใช้จ่ายน้อยที่สุด สำหรับการผลิต ให้ลบการเรียก Ray หรือปิดใช้งาน:
```php
<?php
