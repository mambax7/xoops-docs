...
description: "Kompletny przewodnik do systemu uprawnień XOOPS, typy, sprawdzanie, hierarchia i kontrola dostępu"
---

# System uprawnień w XOOPS

System uprawnień XOOPS to szczegółowa struktura kontroli dostępu, która zarządza tym, kto może wykonać jakie akcje na jakich zasobach. Dokument obejmuje typy uprawnień, mechanizmy sprawdzania, hierarchię i przykłady wdrażania.

## Typy uprawnień

### Uprawnienia na poziomie modułu

Uprawnienia na poziomie modułu kontrolują dostęp do całych modułów lub funkcji modułu.

**Popularne nazwy uprawnień:**
- `module_view` - Wyświetlanie zawartości modułu
- `module_read` - Odczyt zasobów modułu
- `module_submit` - Przesyłanie zawartości do modułu
- `module_edit` - Edycja zawartości modułu
- `module_admin` - Administrowanie modułem
...
### Uprawnienia na poziomie zasobu

Uprawnienia na poziomie zasobu kontrolują dostęp do określonych zasobów w module.

**Przykłady:**
- ID artykułu: Czy grupa może wyświetlać/edytować określony artykuł?
- ID kategorii: Czy grupa może uzyskać dostęp do kategorii?
- ID strony: Czy grupa może wyświetlać/modyfikować określoną stronę?
...
Uprawnienia bloków kontrolują widoczność i interakcję z blokami wyświetlanymi na stronach.
...
Uprawnienia kontrolujące zarządzanie grupami i ich administrację.
...
### Łańcuch dziedziczenia uprawnień
...
Klasa `XoopsGroupPermHandler` zapewnia metody do sprawdzania i zarządzania uprawnieniami.
...
### Sprawdzenie uprawnienia jednego użytkownika
...
### Przykład zarządzania zawartością
...
### Sprawdzenie uprawnień panelu administracyjnego
...
### Zoptymalizowane sprawdzanie uprawnień
...
### Reguły przydzielania uprawnień

1. **Zasada najmniejszych uprawnień**: Przydzielaj tylko niezbędne uprawnienia
2. **Dostęp oparty na rolach**: Używaj grup dla uprawnień opartych na rolach
3. **Regularne audyty**: Okresowo przeglądaj uprawnienia
4. **Rozdzielenie obowiązków**: Rozdzielaj uprawnienia administratora i użytkownika
5. **Jawne odmowy**: Podejście domyślnie zabraniające, jawnie zezwalające
