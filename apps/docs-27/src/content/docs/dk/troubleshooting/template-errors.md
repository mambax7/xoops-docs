---
title: "Skabelonfejl"
description: "Fejlretning og rettelse af Smarty-skabelonfejl i XOOPS"
---

# Skabelonfejl (Smarty Debugging)

> Almindelige Smarty-skabelonproblemer og fejlfindingsteknikker for XOOPS-temaer og -moduler.
...
## Almindelige Smarty-skabelonfejl
...
**Symptomer:**
- "Smarty syntaks fejl" meddelelser
- Skabeloner kompileres ikke
- Tom side uden output
...
### Almindelige syntaksproblemer

**Manglende afsluttende tag:**
```smarty
{* WRONG *}
{if $user}
User: {$user.name}
...
## 2. Udefinerede variable fejl

**Symptomer:**
- "Udefineret variabel" advarsler
- Variable displays som tomme
- PHP-meddelelse i fejllog
...
## 3. Manglende eller forkerte modifikatorer

**Symptomer:**
- Data formateres ikke korrekt
- Tekst vises som HTML
- Forkert kasus/kodning
...
**Registrer tilpasset modifikator:**
...
**Symptomer:**
- Skabelonændringer vises ikke
- Gammelt indhold vises stadig
- Forældet omfatter eller ressourcer
...
## 5. Plugin ikke fundet fejl

**Symptomer:**
- "Ukendt modifikator" eller "Ukendt plugin"
- Brugerdefinerede funktioner virker ikke
- Kompileringsfejl med plugins
...
## 6. Skabelon Inkluder/Udvider problemer

**Symptomer:**
- Medfølgende skabeloner indlæses ikke
- Overordnet skabelon blev ikke fundet
...
## 7. Variabel array/objektadgang

**Symptomer:**
- Kan ikke få adgang til matrixværdier
- Objektegenskaber vises ikke
- Komplekse variable fejler
...
## 8. Problemer med tegnkodning

**Symptomer:**
- Forvansket tekst i skabeloner
- Specialtegn vises forkert
- UTF-8 tegn brudt
...
## Konfiguration af fejlretningstilstand

**Aktiver skabelonfejlretning:**
...
1. **Aktiver debugging** under udvikling
2. **Valider skabeloner** før implementering
3. **Ryd cache** efter ændringer
4. **Brug git** til at spore skabelonændringer
5. **Test i flere browsere** for kodningsproblemer
6. **Dokumentér tilpassede plugins** og modifikatorer
7. **Brug skabelonarv** for konsistens
...
#xoops #fejlfinding #skabeloner #smart #debugging
