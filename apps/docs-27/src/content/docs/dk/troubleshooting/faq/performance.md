---
title: "Ydeevne FAQ"
description: "Ofte stillede spørgsmål om XOOPS ydeevneoptimering"
---

# Ofte stillede spørgsmål om ydeevne

> Almindelige spørgsmål og svar om optimering af XOOPS ydeevne og diagnosticering af langsomme websteder.

---

## Generel præstation

### Q: Hvordan kan jeg se, om mit XOOPS-websted er langsomt?

**A:** Brug disse værktøjer og målinger:
...
### Q: Hvad er de mest almindelige problemer med ydeevnen?
...
### Q: Hvor skal jeg fokusere min optimeringsindsats?

**A:** Følg optimeringsprioriteten:
...
### Sp.: Hvordan aktiverer jeg caching i XOOPS?

**A:** XOOPS har indbygget caching. Konfigurer i Admin > Indstillinger > Ydelse:
...
### Q: Hvilken type caching skal jeg bruge?
...
Aktivér derefter i XOOPS admin.
...
### Q: Hvor længe skal jeg cache data?

**A:** Afhænger af kravene til datafriskhed:
...
### Q: Hvordan kan jeg finde langsomme databaseforespørgsler?

**A:** Aktiver forespørgselslogning:
...
Eller brug MySQL langsom forespørgselslog:
```bash
...
### Q: Hvordan optimerer jeg databaseforespørgsler?
...
**3. Vælg kun nødvendige kolonner**
```php
<?php
// WRONG
$result = $db->query("SELECT * FROM xoops_articles");
...
**5. Brug EXPLAIN til at analysere forespørgsler**
```sql
...
### Q: Hvordan reducerer jeg databasebelastningen?

**A:**
1. **Cache-forespørgselsresultater**:
```php
<?php
$cache = xoops_cache_handler::getInstance();
...
2. **Arkiver gamle data** i separate tabeller
3. **Ryd op i logfiler** regelmæssigt:
```bash
# Delete old log entries (older than 30 days)
DELETE FROM xoops_log WHERE created < NOW() - INTERVAL 30 DAY;
```

4. **Aktiver forespørgselscache** (MySQL):
```sql
...
### Sp.: Hvordan optimerer jeg CSS og JavaScript?
...
**3. Udskyd ikke-kritisk JavaScript**:
```html
...
**4. Aktiver Gzip-komprimering** (.htaccess):
```apache
...
### Q: Hvordan optimerer jeg billeder?
...
### Q: Hvordan kontrollerer jeg serverens ydeevne?
...
### Sp.: Hvordan optimerer jeg PHP til XOOPS?

**A:** Rediger `/etc/php/8.x/fpm/php.ini`:
...
### Q: Hvordan aktiverer jeg HTTP/2 og komprimering?
...
### Q: Hvordan overvåger jeg XOOPS ydeevne over tid?
...
### Q: Hvordan identificerer jeg hukommelseslækager?
