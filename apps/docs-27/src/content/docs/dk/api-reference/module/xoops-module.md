...
description: "Fuldfør API reference for XoopsModule og modulsystemklasser"
---

> Komplet API dokumentation for XOOPS modulsystemet.

---

## Modulsystemarkitektur
...
|--------|------|------------|
| `mid` | int | Modul ID |
| `name` | streng | Vist navn |
| `version` | streng | Versionsnummer |
| `dirname` | streng | Katalognavn |
| `isactive` | int | Aktiv status (0/1) |
| `hasmain` | int | Har hovedområde |
| `hasadmin` | int | Har admin område |
| `hassearch` | int | Har søgefunktion |
| `hasconfig` | int | Har konfiguration |
| `hascomments` | int | Har kommentarer |
| `hasnotification` | int | Har meddelelser |
