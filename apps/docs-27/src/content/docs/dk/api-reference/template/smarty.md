---
title: "Smarty skabelon API reference"
description: "Fuldfør API reference til Smarty-skabelon i XOOPS"
---

> Komplet API-dokumentation til Smarty-skabelon i XOOPS.

---

## Skabelonmotorarkitektur
...
|--------|------------|-------------|
| `assign` | `string $name, mixed $value` | Tildel variabel til skabelon |
| `assignByRef` | `string $name, mixed &$value` | Tildel ved reference |
| `append` | `string $name, mixed $value, bool $merge = false` | Føj til matrixvariabel |
| `display` | `string $template` | Render og output skabelon |
| `fetch` | `string $template` | Gengiv og returner skabelon |
| `clearAssign` | `string $name` | Ryd tildelt variabel |
| `clearAllAssign` | - | Ryd alle variabler |
| `getTemplateVars` | `string $name = null` | Få tildelt variabler |
| `templateExists` | `string $template` | Tjek om skabelonen findes |
| `isCached` | `string $template` | Tjek om skabelonen er cachelagret |
| `clearCache` | `string $template = null` | Ryd skabeloncache |
...
## Brugerdefinerede Smarty Plugins
...
1. **Aktiver skabeloncaching** i produktion
2. **Tildel kun nødvendige variable** - send ikke hele objekter
3. **Brug modifikatorer sparsomt** - forformater i PHP, når det er muligt
4. **Undgå indlejrede sløjfer** - omstrukturer data i PHP
5. **Cache dyre blokke** - brug blokcache til komplekse forespørgsler
...
#xoops #api #smarty #skabeloner #reference
