---
title: "Smarty template API Referensi"
description: "Lengkapi referensi API untuk templating Smarty di XOOPS"
---

> Lengkapi dokumentasi API untuk templating Smarty di XOOPS.

---

## Arsitektur Mesin template
...
|--------|------------|-------------|
| `assign` | `string $name, mixed $value` | Tetapkan variabel ke template |
| `assignByRef` | `string $name, mixed &$value` | Tetapkan dengan referensi |
| `append` | `string $name, mixed $value, bool $merge = false` | Tambahkan ke variabel array |
| `display` | `string $template` | template render dan keluaran |
| `fetch` | `string $template` | Render dan kembalikan template |
| `clearAssign` | `string $name` | Hapus variabel yang ditetapkan |
| `clearAllAssign` | - | Hapus semua variabel |
| `getTemplateVars` | `string $name = null` | Dapatkan variabel yang ditetapkan |
| `templateExists` | `string $template` | Periksa apakah template ada |
| `isCached` | `string $template` | Periksa apakah template di-cache |
| `clearCache` | `string $template = null` | Hapus cache template |
...
1. **Aktifkan cache template** dalam produksi
2. **Tetapkan hanya variabel yang diperlukan** - jangan meneruskan seluruh objek
3. **Gunakan pengubah secara hemat** - pra-format dalam PHP bila memungkinkan
4. **Hindari loop bersarang** - restrukturisasi data di PHP
5. **Cache block mahal** - gunakan cache block untuk kueri kompleks
...
#xoops #api #smarty #template #reference
