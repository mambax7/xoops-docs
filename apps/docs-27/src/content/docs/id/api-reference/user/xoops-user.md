---
title: "Referensi XoopsUser API"
description: "Lengkapi referensi API untuk XoopsUser dan kelas manajemen pengguna"
---

> Lengkapi dokumentasi API untuk sistem pengguna XOOPS.
...
|----------|------|-------------|
| `uid` | ke dalam | ID Pengguna (kunci utama) |
| `uname` | tali | Nama pengguna |
| `name` | tali | Nama asli |
| `email` | tali | Alamat email |
| `pass` | tali | Kata sandi hash |
| `url` | tali | Situs web URL |
| `user_avatar` | tali | Nama file Avatar |
| `user_regdate` | ke dalam | Stempel waktu pendaftaran |
| `user_from` | tali | Lokasi |
| `user_sig` | tali | Tanda tangan |
| `user_occ` | tali | Pekerjaan |
| `user_intrest` | tali | Minat |
| `bio` | tali | Biografi |
| `posts` | ke dalam | Jumlah postingan |
| `rank` | ke dalam | Peringkat pengguna |
| `level` | ke dalam | Tingkat pengguna (0=tidak aktif, 1=aktif) |
| `theme` | tali | theme pilihan |
| `timezone` | mengapung | Pengimbangan zona waktu |
| `last_login` | ke dalam | Stempel waktu login terakhir |
...
| ID Grup | Nama | Deskripsi |
|----------|------|-------------|
| 1 | Webmaster | Akses administratif penuh |
| 2 | Pengguna Terdaftar | Pengguna terdaftar standar |
| 3 | Anonim | Pengunjung yang tidak login |
...
#xoops #api #pengguna #autentikasi #referensi
