---
title: "Prévention de l'injection SQL"
description: "Pratiques de sécurité des bases de données et prévention de l'injection SQL dans XOOPS"
---
...
Si un utilisateur passe `1 OR 1=1` comme ID, la requête devient :
```sql
SELECT * FROM xoops_items WHERE id = 1 OR 1=1
```

Cela retourne tous les enregistrements au lieu d'un seul.
