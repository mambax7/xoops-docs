---
title: "Couche de base de données"
description: "Guide complet de l'abstraction de base de données XOOPS, XoopsObject, handlers et le système Criteria"
---

# 🗄️ Couche de base de données

<span class="version-badge version-25x">2.5.x ✅</span> <span class="version-badge version-40x">4.0.x ✅</span>

> Comprendre l'abstraction de base de données XOOPS, la persistance d'objets et la construction de requêtes.

:::tip[Pérennisez votre accès aux données]
Le modèle handler/Criteria fonctionne dans les deux versions. Pour préparer XOOPS 4.0, envisagez d'envelopper les handlers dans des [classes Repository](../../03-Module-Development/Patterns/Repository-Pattern.md) pour une meilleure testabilité. Voir [Choosing a Data Access Pattern](../../03-Module-Development/Choosing-Data-Access-Pattern.md).
:::
...
La couche de base de données XOOPS fournit une abstraction robuste sur MySQL/MariaDB, comportant :

- **Modèle factory** - Gestion centralisée des connexions de base de données
- **Cartographie objet-relationnelle** - XoopsObject et handlers
- **Construction de requêtes** - Système Criteria pour les requêtes complexes
- **Réutilisation de connexion** - Connexion unique via singleton factory (pas de pooling)
...
Le modèle factory garantit qu'une connexion de base de données unique est réutilisée :
...
|----------|------|-------------|
| `XOBJ_DTYPE_INT` | Entier | Valeurs numériques |
| `XOBJ_DTYPE_TXTBOX` | String | Texte court (< 255 caractères) |
| `XOBJ_DTYPE_TXTAREA` | Texte | Contenu de texte long |
| `XOBJ_DTYPE_EMAIL` | Email | Adresses e-mail |
| `XOBJ_DTYPE_URL` | URL | Adresses Web |
| `XOBJ_DTYPE_FLOAT` | Float | Nombres décimaux |
| `XOBJ_DTYPE_ARRAY` | Tableau | Tableaux sérialisés |
| `XOBJ_DTYPE_OTHER` | Mixte | Données brutes |
...
La classe handler gère les opérations CRUD pour les instances XoopsObject.
...
### Méthodes Handler personnalisées
...
Le système Criteria fournit un moyen puissant et orienté objet de construire des clauses SQL WHERE.
...
### CriteriaCompo (Combinaison de Criteria)
...
Pour les requêtes complexes non possibles avec Criteria, utilisez le SQL direct.

### Requêtes sûres (lecture)
...
### Toujours échapper l'entrée utilisateur
...
### Utiliser les requêtes paramétrées (XMF)
