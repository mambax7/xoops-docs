---
title: "Modèles de conception dans XOOPS"
description: "Guide complet des modèles de conception utilisés dans le développement XOOPS incluant MVC, Singleton, Factory, Observer, et plus"
---
...
:::note[Sélection rapide de modèles]
Vous ne savez pas quel modèle utiliser ? Consultez :
- [Choisir un modèle d'accès aux données](../../03-Module-Development/Choosing-Data-Access-Pattern.md) — handlers vs repository vs service vs CQRS
- [Choisir un système d'événements](../Choosing-Event-System.md) — preloads vs événements PSR-14
:::

## Vue d'ensemble

Comprendre et implémenter correctement les modèles de conception est crucial pour créer des modules XOOPS maintenables. Ce guide couvre les modèles les plus couramment utilisés dans le développement XOOPS.
...
Le modèle MVC sépare une application en trois composants interconnectés, rendant la base de code plus organisée et testable.

### Architecture

```mermaid
flowchart TB
    subgraph MVC["Modèle MVC dans XOOPS"]
        Controller["🎮 Contrôleur<br/>(index.php, admin/index.php)"]
        Model["📦 Modèle<br/>(Handlers)"]
        View["🎨 Vue<br/>(Modèles)"]
...
Le modèle Singleton garantit qu'une classe n'a qu'une seule instance et fournit un accès global à celle-ci.
...
Le modèle Factory crée des objets sans spécifier leur classe exacte, permettant une création d'objets flexible.

### Quand l'utiliser

- Création de handlers de manière dynamique
...
### Factory de base de données XOOPS
...
Le modèle Observer permet aux objets d'être notifiés des changements d'état d'un sujet, permettant un comportement piloté par les événements.
...
### Preloads XOOPS (Implémentation Observer)
...
Le modèle Decorator ajoute un comportement aux objets de manière dynamique sans affecter les autres objets de la même classe.
...
Le modèle Strategy définit une famille d'algorithmes, encapsule chacun d'eux et les rend interchangeables.
...
Le modèle Repository fournit une couche d'abstraction entre la logique d'accès aux données et la logique métier.

### Quand l'utiliser

- Exigences complexes d'accès aux données
...
L'injection de dépendances (DI) permet aux objets d'être construits avec leurs dépendances au lieu de les créer en interne.
...
### Directives de sélection de modèles

1. **Choisir les modèles en fonction des besoins réels**, pas des besoins anticipés
2. **Garder les implémentations simples** - ne pas sur-concevoir
3. **Documenter l'utilisation des modèles** pour la compréhension de l'équipe
4. **Combiner les modèles** si approprié (par exemple, Factory + Singleton)
5. **Considérer la testabilité** lors de la sélection des modèles
...
- [Architecture XOOPS](XOOPS-Architecture.md) - Architecture globale du système
- [Couche de base de données](../Database/Database-Layer.md) - Modèles de persistance des données
- [Bonnes pratiques de sécurité](../Security/Security-Best-Practices.md) - Implémentation sécurisée des modèles
