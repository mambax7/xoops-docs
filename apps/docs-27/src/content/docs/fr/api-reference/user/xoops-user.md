---
title: "Référence API XoopsUser"
description: "Référence API complète pour la classe XoopsUser et les classes de gestion des utilisateurs"
---

> Documentation API complète pour le système d'utilisateurs XOOPS.
...
|----------|------|-------------|
| `uid` | int | ID utilisateur (clé primaire) |
| `uname` | string | Nom d'utilisateur |
| `name` | string | Nom réel |
| `email` | string | Adresse email |
| `pass` | string | Hachage du mot de passe |
| `url` | string | URL du site web |
| `user_avatar` | string | Nom du fichier avatar |
| `user_regdate` | int | Horodatage d'enregistrement |
| `user_from` | string | Localisation |
| `user_sig` | string | Signature |
| `user_occ` | string | Profession |
| `user_intrest` | string | Intérêts |
| `bio` | string | Biographie |
| `posts` | int | Nombre de messages |
| `rank` | int | Rang utilisateur |
| `level` | int | Niveau utilisateur (0=inactif, 1=actif) |
| `theme` | string | Thème préféré |
| `timezone` | float | Décalage de fuseau horaire |
| `last_login` | int | Horodatage de la dernière connexion |
...
|----------|------|-------------|
| 1 | Webmasters | Accès administratif complet |
| 2 | Registered Users | Utilisateurs enregistrés standard |
| 3 | Anonymous | Visiteurs non connectés |
