---
title: "Построение модуля CRUD"
description: "Полное руководство по созданию модуля CRUD с операциями базы данных, формами и интерфейсом администратора"
---

# Руководство по построению модуля CRUD

Это руководство проведет вас через построение полного модуля CRUD (Create, Read, Update, Delete) для XOOPS. Мы создадим модуль "Notes", который позволяет пользователям управлять личными заметками.

## Предусловия

- Завершено руководство Hello-World-Module
- Понимание концепций PHP OOP
- Базовые знания SQL
...
- Заметки, специфичные для пользователя
- Организация по категориям
- Функциональность поиска

## Шаг 1: Структура каталогов

Создайте следующую структуру в `/modules/notes/`:
...
Создайте `sql/mysql.sql`:

```sql
-- Схема базы данных модуля Notes
...
## Шаг 4: Классы сущностей

### Entity Note

Создайте `class/Note.php`:
...
Создайте `class/NoteHandler.php`:
...
Создайте `class/CategoryHandler.php`:
...
## Шаг 5: Общий файл включения

Создайте `include/common.php`:
...
### Страница редактирования (Создание/Обновление)
...
Создайте `templates/notes_index.tpl`:
...
Создайте `templates/notes_view.tpl`:
...
Создайте `templates/notes_edit.tpl`:
...
### Управление заметками администратора

Создайте `admin/notes.php`:
...
Поздравляем! Вы построили полный модуль CRUD. Охватанные ключевые концепции:

1. **Database Design** - Таблицы с отношениями
2. **Entity Classes** - XoopsObject с типизированными свойствами
3. **Handler Classes** - XoopsPersistableObjectHandler с пользовательскими методами
4. **Frontend Pages** - Функциональность списка, просмотра и редактирования
5. **Form Handling** - Защита CSRF и валидация
6. **Admin Interface** - Экраны управления
7. **Templates** - Шаблоны Smarty с логикой

## Следующие шаги

- Добавьте более продвинутые функции (комментарии, рейтинги, обмен)
- Реализуйте ../Patterns/Repository-Pattern для более чистого доступа к данным
- Примените ../Patterns/MVC-Pattern для лучшей организации кода
- Добавьте ../Best-Practices/Testing с PHPUnit
