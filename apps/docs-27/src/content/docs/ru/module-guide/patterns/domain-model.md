---
title: "Паттерн Domain Model"
---

## Обзор

Паттерн Domain Model представляет бизнес-концепции, правила и логику вашего приложения. При разработке модулей XOOPS моделями предметной области инкапсулируются основные бизнес-сущности и их поведение.
...
Value Objects являются неизменяемыми и сравниваются по значению:
...
Aggregates — это кластеры объектов предметной области, рассматриваемые как единица:
...
Захватывают важные события в предметной области:
...
Используйте перечисления PHP 8.2+ для безопасных по типам значений статуса:
...
Защитите правила предметной области в entities:
...
1. **Rich Domain Model** - Поместите поведение в entities, а не в services
2. **Immutable Value Objects** - Value objects никогда не должны изменяться
3. **Factory Methods** - Используйте статические фабричные методы для сложного создания
4. **Guard Clauses** - Валидируйте входные данные на границах entity
5. **Domain Events** - Захватывайте значительные изменения состояния
6. **Ubiquitous Language** - Используйте бизнес-терминологию в коде
