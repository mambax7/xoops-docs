---
title: "Пользовательские рендеры форм"
---

## Обзор

XOOPS позволяет настраивать рендеринг форм через пользовательские рендеры. Это обеспечивает стилизацию специфичную для темы, улучшения доступности и интеграцию с фреймворками фронтенда, такими как Bootstrap или Tailwind CSS.

## Рендеринг по умолчанию

По умолчанию формы XOOPS используют класс `XoopsFormRenderer`, который выводит базовый HTML:
