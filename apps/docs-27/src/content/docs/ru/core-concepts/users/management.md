...
description: "Полное руководство по системе пользователей XOOPS, регистрации, аутентификации и управлению профилем"
---

# Управление пользователями в XOOPS

Система управления пользователями XOOPS предоставляет полный набор функций для обработки регистрации пользователей, аутентификации, управления профилем и предпочтений пользователей. Этот документ охватывает структуру объекта пользователя, потоки регистрации и практические примеры реализации.

## Структура объекта пользователя

Основной объект пользователя в XOOPS — это класс `XoopsUser`, который инкапсулирует все данные пользователя и методы.
...
- Всегда используйте `password_hash()` с алгоритмом `PASSWORD_BCRYPT`
- Используйте параметр cost равный 12 для bcrypt
