---
title: "مرجع API الگوی هوشمند"
description: "مرجع کامل API برای قالب Smarty در XOOPS"
---
> اسناد API کامل برای قالب Smarty در XOOPS.
...
| روش | پارامترها | توضیحات |
|--------|-----------|-------------|
| `assign` | `string $name, mixed $value` | اختصاص متغیر به الگو |
| `assignByRef` | `string $name, mixed &$value` | تخصیص با مرجع |
| `append` | `string $name, mixed $value, bool $merge = false` | افزودن به متغیر آرایه |
| `display` | `string $template` | قالب رندر و خروجی |
| `fetch` | `string $template` | قالب رندر و برگرداندن |
| `clearAssign` | `string $name` | پاک کردن متغیر اختصاص داده شده |
| `clearAllAssign` | - | پاک کردن همه متغیرها |
| `getTemplateVars` | `string $name = null` | دریافت متغیرهای اختصاص داده شده |
| `templateExists` | `string $template` | بررسی کنید که آیا الگو وجود دارد |
| `isCached` | `string $template` | بررسی کنید که آیا الگو ذخیره شده است |
| `clearCache` | `string $template = null` | پاک کردن کش قالب |
...
1. **فعال کردن کش قالب** در تولید
2. **فقط متغیرهای مورد نیاز را اختصاص دهید ** - تمام اشیاء را رد نکنید
3. **از اصلاح کننده ها به مقدار کم استفاده کنید** - در صورت امکان از پیش فرمت در PHP
4. **از حلقه های تو در تو اجتناب کنید** - داده ها را در PHP بازسازی کنید
5. **بلاک های گران قیمت را در حافظه پنهان ذخیره کنید** - از ذخیره بلاک برای پرس و جوهای پیچیده استفاده کنید
