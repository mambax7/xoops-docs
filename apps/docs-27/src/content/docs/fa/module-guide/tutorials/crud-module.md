...
description: "آموزش کامل ساخت ماژول CRUD با عملیات پایگاه داده، فرم ها و رابط مدیریت"
---
# آموزش ساخت ماژول CRUD

این آموزش شما را در ساخت یک ماژول کامل CRUD (ایجاد، خواندن، به‌روزرسانی، حذف) برای XOOPS راهنمایی می‌کند. ما یک ماژول "یادداشت ها" ایجاد خواهیم کرد که به کاربران اجازه می دهد یادداشت های شخصی را مدیریت کنند.

## پیش نیاز

- آموزش کامل Hello-World-Module
- درک مفاهیم PHP OOP
- دانش اولیه SQL

## مرور کلی ماژول

**ویژگی های ماژول یادداشت ها:**
- ایجاد، مشاهده، ویرایش، و حذف یادداشت ها
- رابط مدیریت مدیریت
- یادداشت های خاص کاربر
- سازمان طبقه بندی
- قابلیت جستجو

## مرحله 1: ساختار دایرکتوری

ساختار زیر را در `/modules/notes/` ایجاد کنید:
...
## مرحله 2: طرح واره پایگاه داده

`sql/mysql.sql` را ایجاد کنید:
...
## مرحله 4: کلاس های نهاد

### نهاد یادداشت

`class/Note.php` را ایجاد کنید:
...
`class/NoteHandler.php` را ایجاد کنید:
...
`class/Category.php` را ایجاد کنید:
...
`class/CategoryHandler.php` را ایجاد کنید:
...
## مرحله 5: Common Include File

`include/common.php` را ایجاد کنید:
...
## مرحله 6: صفحات فرانت اند

### صفحه فهرست

`index.php` را ایجاد کنید:
...
### ویرایش صفحه (Create/Update)

`edit.php` را ایجاد کنید:
...
`templates/notes_index.tpl` را ایجاد کنید:
...
`templates/notes_view.tpl` را ایجاد کنید:
...
`templates/notes_edit.tpl` را ایجاد کنید:
...
### مدیریت یادداشت های مدیریت

`admin/notes.php` را ایجاد کنید:
...
تبریک می گویم! شما یک ماژول کامل CRUD ساخته اید. مفاهیم کلیدی تحت پوشش:

1. **طراحی پایگاه داده** - جداول با روابط
2. **کلاس های نهاد** - XoopsObject با ویژگی های تایپ شده
3. ** کلاس های Handler ** - XoopsPersistableObjectHandler با روش های سفارشی
4. **صفحات جلویی** - قابلیت لیست، مشاهده و ویرایش
5. ** مدیریت فرم ** - حفاظت و اعتبار CSRF
6. **رابط مدیریت ** - صفحه نمایش مدیریت
7. **قالب ها** - قالب های هوشمند با منطق

## مراحل بعدی

- اضافه کردن ویژگی های پیشرفته تر (نظرات، رتبه بندی، اشتراک گذاری)
- ../Patterns/Repository-Pattern را برای دسترسی تمیزتر به داده ها پیاده سازی کنید
- برای سازماندهی بهتر کد، ../Patterns/MVC-Pattern را اعمال کنید
- ../Best-Practices/Testing را با PHPUnit اضافه کنید

همچنین ببینید: ../Module-Development | ../Patterns/MVC-Pattern | ../Patterns/Repository-Pattern