...
ساختار ماژول به خوبی سازماندهی شده برای توسعه پایدار XOOPS اساسی است. این راهنما هر دو طرح‌بندی ماژول قدیمی و مدرن (PSR-4) را پوشش می‌دهد.

## چیدمان استاندارد ماژول
...
### xoops_version.php (مانیفست میراث)
...
|-----------|---------|
| `admin/` | رابط مدیریت |
| `assets/` | CSS، جاوا اسکریپت، تصاویر |
| `blocks/` | توابع رندر بلوک |
| `class/` | کلاس های PHP (میراث) |
| `config/` | فایل های پیکربندی (مدرن) |
| `include/` | شامل فایل های مشترک |
| `language/` | فایل های ترجمه |
| `migrations/` | مهاجرت های پایگاه داده |
| `sql/` | طرح اولیه پایگاه داده |
| `src/` | کد منبع PSR-4 |
| `templates/` | قالب های هوشمند |
| `tests/` | فایل های تست |

## بهترین شیوه ها

1. **نگرانی های جداگانه ** - منطق کسب و کار را از قالب ها دور نگه دارید
2. **استفاده از فضاهای نام** - کدها را با فاصله نامی مناسب سازماندهی کنید
3. ** PSR-4 ** را دنبال کنید - از قراردادهای استاندارد بارگذاری خودکار استفاده کنید
4. ** Externalize Config ** - پیکربندی را جدا از کد نگه دارید
5. **ساختار سند ** - شامل سازمان توضیح دهنده README

## مستندات مرتبط

- ماژول-توسعه - راهنمای توسعه کامل
- Best-Practices/Code-Organization - الگوهای سازماندهی کد
- ماژول مانیفست - پیکربندی مانیفست
- Database/Database-Schema - طراحی پایگاه داده