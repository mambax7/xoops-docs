---
title: "קריטריונים API הפניה"
description: "השלם הפניה של API עבור מערכת בניית השאילתות XOOPS קריטריונים"
---
> תיעוד מלא API עבור מערכת בניית השאילתות XOOPS קריטריונים.

---

## ארכיטקטורת מערכת קריטריונים
```mermaid
classDiagram
...
1. **הגדר תמיד LIMIT** לשולחנות גדולים
2. **השתמש באינדקסים** על עמודות המשמשות בקריטריונים
3. **הימנע מתווים כלליים מובילים** ב-LIKE (`'%term'` איטי)
4. **מסנן מראש ב-PHP** כשאפשר להיגיון מורכב
5. **השתמש ב-COUNT במשורה** - תוצאות cache במידת האפשר
...
#xoops #api #קריטריונים #בסיס נתונים #שאילתה #הפניה