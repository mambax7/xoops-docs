---
title: "XoopsForm API הפניה"
description: "השלם הפניה API עבור XoopsForm ומחלקות רכיבי טופס"
---
> תיעוד מלא API עבור מערכת יצירת הטפסים XOOPS.
...
|--------|------------|--------|--------------|
| `addElement` | `XoopsFormElement $element, bool $required = false` | `void` | הוסף אלמנט לטופס |
| `getElements` | - | `array` | קבל את כל האלמנטים |
| `getElement` | `string $name` | `XoopsFormElement|null` | קבל אלמנט לפי שם |
| `setExtra` | `string $extra` | `void` | הגדר תכונות נוספות של HTML |
| `getExtra` | - | `string` | קבל תכונות נוספות |
| `getTitle` | - | `string` | קבל כותרת טופס |
| `setTitle` | `string $title` | `void` | הגדר כותרת טופס |
| `getName` | - | `string` | קבל שם טופס |
| `getAction` | - | `string` | קבל פעולה URL |
| `render` | - | `string` | טנדר טופס HTML |
| `display` | - | `void` | טופס שניתנו הד |
| `insertBreak` | `string $extra = ''` | `void` | הכנס הפסקה חזותית |
| `setRequired` | `XoopsFormElement $element` | `void` | סמן רכיב נדרש |

---

## XoopsThemeForm

מחלקת הטפסים הנפוצה ביותר, מעובדת עם סגנון מודע לנושא.
...
תיבת סימון או קבוצת תיבת סימון.
```php
...
#xoops #api #טופס #xoopsform #reference