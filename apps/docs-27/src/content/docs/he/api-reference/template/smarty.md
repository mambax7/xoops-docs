---
title: "Smarty תבנית API הפניה"
description: "השלם הפניה API עבור תבנית Smarty ב-XOOPS"
---

> תיעוד מלא API עבור תבנית Smarty ב-XOOPS.

---

## ארכיטקטורת מנוע תבנית
...
| שיטה | פרמטרים | תיאור |
|--------|---------------------------|
| `assign` | `string $name, mixed $value` | הקצה משתנה לתבנית |
| `assignByRef` | `string $name, mixed &$value` | הקצה לפי הפניה |
| `append` | `string $name, mixed $value, bool $merge = false` | הוסף למשתנה מערך |
| `display` | `string $template` | עיבוד ופלט תבנית |
| `fetch` | `string $template` | תבנית עיבוד והחזרה |
| `clearAssign` | `string $name` | נקה משתנה מוקצה |
| `clearAllAssign` | - | נקה את כל המשתנים |
| `getTemplateVars` | `string $name = null` | קבל משתנים שהוקצו |
| `templateExists` | `string $template` | בדוק אם קיימת תבנית |
| `isCached` | `string $template` | בדוק אם התבנית מאוחסנת בcache |
| `clearCache` | `string $template = null` | נקה cache תבנית |
...
## תוספים מותאמים אישית של Smarty
...
1. **אפשר שמירה בcache של תבנית** בייצור
2. **הקצה רק משתנים נחוצים** - אל תעביר אובייקטים שלמים
3. **השתמש במדיפיקטורים במשורה** - פורמט מראש ב-PHP במידת האפשר
4. **הימנע מלולאות מקוננות** - מבנה מחדש נתונים ב-PHP
5. **cache בלוקים יקרים** - השתמש ב-block caching עבור שאילתות מורכבות
