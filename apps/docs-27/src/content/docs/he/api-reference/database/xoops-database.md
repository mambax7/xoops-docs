...
description: "שכבת הפשטת מסד נתונים מספקת ניהול חיבורים, ביצוע שאילתות וטיפול בתוצאות"
---
...
| פרמטר | הקלד | תיאור |
|-----------|------|------------|
| `$selectdb` | bool | האם לבחור את מסד הנתונים |
...
| פרמטר | הקלד | תיאור |
|-----------|------|------------|
| `$sql` | מחרוזת | מחרוזת שאילתה SQL |
| `$limit` | int | מקסימום שורות להחזרה (0 = ללא הגבלה) |
| `$start` | int | קיזוז התחלתי |
...
| פרמטר | הקלד | תיאור |
|-----------|------|------------|
| `$result` | משאב | משאב תוצאות שאילתה |
...
2. **השתמש בהצהרות מוכנות כאשר הן זמינות**:
```php
$stmt = $db->prepare("SELECT * FROM users WHERE id = ?");
$stmt->bind_param('i', $id);
```

3. **השתמש ב-quoteString עבור ערכים**:
```php
...
2. **ערכות תוצאות חינמיות בסיום**:
```php
$db->freeRecordSet($result);
```

3. **השתמש באינדקסים מתאימים** בהגדרות הטבלה שלך

4. **העדיפו את המטפלים על פני SQL הגולמי** במידת האפשר
...
2. **השתמש בעסקאות למספר פעולות קשורות**:
```php
$db->beginTransaction();
// ... operations ...
...
*ראה גם: [קוד מקור XOOPS](https://github.com/XOOPS/XoopsCore27/tree/master/htdocs/class/database)*
