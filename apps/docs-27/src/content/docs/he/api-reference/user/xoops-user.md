---
title: "XoopsUser API הפניה"
description: "השלם הפניה של API עבור XoopsUser ושיעורי ניהול משתמשים"
---
> תיעוד מלא API עבור מערכת המשתמש XOOPS.
...
| נכס | הקלד | תיאור |
|--------|------|--------|
| `uid` | int | מזהה משתמש (מפתח ראשי) |
| `uname` | מחרוזת | שם משתמש |
| `name` | מחרוזת | שם אמיתי |
| `email` | מחרוזת | כתובת דואר אלקטרוני |
| `pass` | מחרוזת | סיסמא hash |
| `url` | מחרוזת | אתר אינטרנט URL |
| `user_avatar` | מחרוזת | שם קובץ אווטאר |
| `user_regdate` | int | חותמת זמן רישום |
| `user_from` | מחרוזת | מיקום |
| `user_sig` | מחרוזת | חתימה |
| `user_occ` | מחרוזת | עיסוק |
| `user_intrest` | מחרוזת | תחומי עניין |
| `bio` | מחרוזת | ביוגרפיה |
| `posts` | int | ספירת פוסטים |
| `rank` | int | דירוג משתמש |
| `level` | int | רמת משתמש (0=לא פעיל, 1=פעיל) |
| `theme` | מחרוזת | נושא מועדף |
| `timezone` | לצוף | היסט אזור זמן |
| `last_login` | int | חותמת זמן כניסה אחרונה |
...
| מזהה קבוצה | שם | תיאור |
|--------|------|--------|
| 1 | מנהלי אתרים | גישה מנהלתית מלאה |
| 2 | משתמשים רשומים | משתמשים רשומים רגילים |
| 3 | אנונימי | מבקרים לא מחוברים |
