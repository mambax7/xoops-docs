---
title: "מערכת הרשאות"
description: "מדריך שלם למערכת הרשאות XOOPS, סוגים, בדיקה, היררכיה ובקרת גישה"
---
# מערכת הרשאות ב- XOOPS

מערכת ההרשאות XOOPS היא מסגרת בקרת גישה מפורטת שמנהלת מי יכול לבצע אילו פעולות באילו משאבים. מסמך זה מכסה סוגי הרשאות, מנגנוני בדיקה, היררכיה ודוגמאות יישום.

## סוגי הרשאות

### הרשאות ברמת מודול

הרשאות ברמת המודול שולטות בגישה למודולים שלמים או לפונקציות של מודול.

**שמות ההרשאות הנפוצות:**
- `module_view` - צפה בתוכן המודול
- `module_read` - קרא משאבי מודול
- `module_submit` - שלח תוכן למודול
- `module_edit` - עריכת תוכן המודול
...
הרשאות ברמת הפריט שולטות בגישה למשאבים ספציפיים בתוך מודול.

**דוגמאות:**
- מזהה מאמר: האם ניתן לקבץ view/edit מאמר ספציפי?
- מזהה קטגוריה: האם ניתן לגשת לקטגוריה של קבוצה?
- מזהה עמוד: האם ניתן לקבץ view/modify עמוד ספציפי?
```php
<?php
/**
...
הרשאות חסימה שולטות בנראות ובאינטראקציה עם בלוקים המוצגים בדפים.
```php
<?php
/**
...
הרשאות שליטה בניהול וניהול קבוצות.
```php
<?php
/**
...
המחלקה `XoopsGroupPermHandler` מספקת שיטות לבדיקה וניהול של הרשאות.
```php
<?php
/**
...
```
## שמירה בcache של הרשאות

### בדיקת הרשאות אופטימלית
```php
<?php
/**
...
1. **עקרון הרשאות הכי פחות**: הקצה הרשאות נחוצות בלבד
2. **גישה מבוססת תפקידים**: השתמש בקבוצות להרשאות מבוססות תפקידים
3. **ביקורות רגילות**: בדיקה תקופתית של הרשאות
4. **הפרדת חובות**: הפרד את מנהל המערכת מהרשאות המשתמש
5. **הכחשה מפורשת**: דחיית ברירת מחדל, גישה מפורשת לאפשר
