---
title: "CSS שיטות עבודה מומלצות עבור XOOPS ערכות נושא"
---
## סקירה כללית

מדריך זה מכסה CSS ארגון, טכניקות פריסה מודרניות ואופטימיזציה של ביצועים לפיתוח נושא XOOPS.
...
1. **צמצם את הספציפיות** - הימנע מקינון עמוק
2. **השתמש במאפיינים מודרניים** - `gap` במקום שוליים
3. **הפחת צביעה חוזרת** - השתמש ב-`transform` עבור אנימציות
4. **עומס עצלן** - טען מודול CSS רק בעת הצורך
```css
...
- Theme-Structure - ארגון קבצי נושא
- פיתוח נושא - מדריך ערכות נושא מלא
- ../Templates/Smarty-Templating - שילוב תבניות
- ../../03-Module-Development/Block-Development - עיצוב בלוק