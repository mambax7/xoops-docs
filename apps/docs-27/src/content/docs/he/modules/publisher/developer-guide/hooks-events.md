---
title: "Publisher - הוקס ואירועים"
description: "מדריך להרחבת Publisher באמצעות הוקס, אירועים ומערכת תוספים"
---
# הוקס ואירועים של מוציאים לאור

> מדריך שלם להרחבת הפונקציונליות של Publisher באמצעות אירועים, הוקס ותוספים.
...
אירועים מאפשרים למודולים אחרים להגיב לפעולות של בעל אתר:
```
...
מופעל כאשר סטטוס המאמר משתנה לפרסום.
```php
...
```
#### publisher.category.עדכן

מופעל כאשר הקטגוריה מתעדכנת.
```php
...
```
#### publisher.category.מחק

מופעל כאשר הקטגוריה נמחקת.
```php
...
```
#### publisher.comment.מחק

מופעל כאשר תגובה נמחקת.
```php
...
### דוגמה 1: שלח אימייל בעת יצירת מאמר
```php
<?php
// Listen for article creation
xoops_events()->attach(
...
?>
```
### דוגמה 3: פרסום אוטומטי למדיה חברתית
```php
<?php
...
?>
```
### דוגמה 4: סנכרון עם מערכת חיצונית
```php
<?php
...
הוקס מאפשרים שינויים בהתנהגות של בעל אתר:

#### publisher.view.article.start

נקרא לפני עיבוד המאמר.
```php
xoops_events()->attach(
    'publisher.view.article.start',
...
תוספים מרחיבים את הפונקציונליות של Publisher:
...
מסננים משנים נתונים before/after עיבוד:
```php
<?php
...
- [XOOPS מערכת אירועים](../../03-Module-Development/Module-Development.md)
- [פיתוח פלאגין](../../03-Module-Development/Module-Development.md)
