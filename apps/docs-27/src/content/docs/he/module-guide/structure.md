...
מבנה מודול מאורגן היטב הוא הבסיס לפיתוח XOOPS בר תחזוקה. מדריך זה מכסה הן פריסות מודול מדור קודם והן מודרניות (PSR-4).

## פריסת מודול סטנדרטית
...
### xoops_version.php (מניפסט מדור קודם)
```php
<?php
$modversion = [
    'name'           => 'My Module',
...
| `blocks/` | פונקציות עיבוד חסימה |
| `class/` | PHP שיעורים (מורשת) |
| `config/` | קבצי תצורה (מודרניים) |
| `include/` | כולל קבצים משותפים |
| `language/` | קבצי תרגום |
| `migrations/` | העברת מסדי נתונים |
| `sql/` | סכימת מסד נתונים ראשונית |
| `src/` | PSR-4 קוד מקור |
| `templates/` | Smarty תבניות |
| `tests/` | קבצי בדיקה |

## שיטות עבודה מומלצות

1. **חששות נפרדים** - הרחיקו את ההיגיון העסקי מתבניות
2. **השתמש במרחבי שמות** - ארגן קוד עם מרווח שמות מתאים
3. **עקוב אחר PSR-4** - השתמש במוסכמות הטעינה האוטומטית הסטנדרטיות
4. **החצן תצורה** - שמור על תצורה נפרדת מהקוד
5. **מבנה מסמך** - כלול README ארגון מסביר

## תיעוד קשור

- פיתוח מודול - מדריך פיתוח מלא
- Best-Practices/Code-Organization - דפוסי ארגון קוד
- Manifest Manifest - תצורת מניפסט
- Database/Database-Schema - עיצוב מסד נתונים