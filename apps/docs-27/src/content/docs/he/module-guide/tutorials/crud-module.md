---
title: "בניית מודול CRUD"
description: "הדרכה מלאה לבניית מודול CRUD עם פעולות מסד נתונים, טפסים וממשק ניהול"
---
# בניית הדרכה CRUD מודול

מדריך זה מנחה אותך בבניית מודול CRUD (צור, קרא, עדכן, מחק) שלם עבור XOOPS. ניצור מודול "הערות" המאפשר למשתמשים לנהל הערות אישיות.

## דרישות מוקדמות

- הושלם הדרכה של Hello-World-Module
- הבנה של מושגי PHP OOP
- ידע בסיסי SQL

## סקירת מודול

**תכונות מודול הערות:**
- צור, הצג, ערוך ומחק הערות
- ממשק ניהול אדמין
- הערות ספציפיות למשתמש
- ארגון קטגוריות
- פונקציונליות חיפוש

## שלב 1: מבנה ספריות

צור את המבנה הבא ב-`/modules/notes/`:
```
/modules/notes/
    /admin/
        admin_header.php
...
```
## שלב 5: קובץ כלול נפוץ

צור `include/common.php`:
```php
<?php
/**
 * Common Include File
...
מזל טוב! בנית מודול CRUD שלם. מושגי מפתח מכוסים:

1. **עיצוב מסד נתונים** - טבלאות עם קשרים
2. **מחלקות ישות** - XoopsObject עם מאפיינים מוקלדים
3. **שיעורי מטפל** - XoopsPersistableObjectHandler בשיטות מותאמות אישית
4. **דפי חזית** - רשימה, הצג וערוך פונקציונליות
5. **טיפול בטפסים** - CSRF הגנה ואימות
6. **ממשק ניהול** - מסכי ניהול
7. **תבניות** - Smarty תבניות עם היגיון

## השלבים הבאים

- הוסף תכונות מתקדמות יותר (הערות, דירוגים, שיתוף)
- יישם את ../Patterns/Repository-Pattern לגישה נקייה יותר לנתונים
- החל ../Patterns/MVC-Pattern לארגון קוד טוב יותר
- הוסף ../Best-Practices/Testing עם PHPUnit

ראה גם: ../Module-Development | ../Patterns/MVC-Pattern | ../Patterns/Repository-Pattern