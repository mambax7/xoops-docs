...
description: "Popolna referenca API za XoopsUser in razrede upravljanja uporabnikov"
---
> Popolna API dokumentacija za XOOPS uporabniški sistem.
...
| Lastnina | Vrsta | Opis |
|----------|------|-------------|
| `uid` | int | ID uporabnika (primarni ključ) |
| `uname` | niz | Uporabniško ime |
| `name` | niz | Pravo ime |
| `email` | niz | E-poštni naslov |
| `pass` | niz | Zgoščena vrednost gesla |
| `url` | niz | Spletna stran URL |
| `user_avatar` | niz | Ime datoteke avatarja |
| `user_regdate` | int | Časovni žig registracije |
| `user_from` | niz | Lokacija |
| `user_sig` | niz | Podpis |
| `user_occ` | niz | Poklic |
| `user_intrest` | niz | Zanimanja |
| `bio` | niz | Biografija |
| `posts` | int | Število objav |
| `rank` | int | Uvrstitev uporabnika |
| `level` | int | Raven uporabnika (0=neaktivno, 1=aktivno) |
| `theme` | niz | Prednostna tema |
| `timezone` | lebdeti | Odmik časovnega pasu |
| `last_login` | int | Časovni žig zadnje prijave |
...
| ID skupine | Ime | Opis |
|----------|------|-------------|
| 1 | Spletni mojstri | Popoln skrbniški dostop |
| 2 | Registrirani uporabniki | Standardni registrirani uporabniki |
| 3 | Anonimno | Neprijavljeni obiskovalci |
...
- Navodila za upravljanje uporabnikov
- Sistem dovoljenj
- Preverjanje pristnosti
