---
title: "Predloga Smarty API Referenca"
description: "Popolna referenca API za predloge Smarty v XOOPS"
---
> Popolna API dokumentacija za predloge Smarty v XOOPS.

---

## Arhitektura mehanizma predloge
```mermaid
graph TB
...
|--------|------------|-------------|
| `assign` | `string $name, mixed $value` | Dodeli spremenljivko predlogi |
| `assignByRef` | `string $name, mixed &$value` | Dodeli s sklicevanjem |
| `append` | `string $name, mixed $value, bool $merge = false` | Dodaj k spremenljivki polja |
| `display` | `string $template` | Upodabljanje in izhodna predloga |
| `fetch` | `string $template` | Predloga upodabljanja in vračila |
| `clearAssign` | `string $name` | Počisti dodeljeno spremenljivko |
| `clearAllAssign` | - | Počisti vse spremenljivke |
| `getTemplateVars` | `string $name = null` | Pridobi dodeljene spremenljivke |
| `templateExists` | `string $template` | Preverite, ali predloga obstaja |
| `isCached` | `string $template` | Preverite, ali je predloga predpomnjena |
| `clearCache` | `string $template = null` | Počisti predpomnilnik predlog |
...
<{/foreach}>
```
### Ravnanje s slikami in sredstvi
```smarty
{* Module image *}
...
1. **Omogoči predpomnjenje predlog** v proizvodnji
2. **Dodeli samo potrebne spremenljivke** - ne posreduj celih objektov
3. **Uporabljajte modifikatorje zmerno** - vnaprej formatirajte v PHP, kadar je to mogoče
4. **Izogibajte se ugnezdenim zankam** - prestrukturirajte podatke v PHP
5. **Predpomnite drage bloke** - uporabite predpomnjenje blokov za zapletene poizvedbe
