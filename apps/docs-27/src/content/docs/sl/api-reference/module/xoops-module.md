...
description: "Popolna referenca API za XoopsModule in sistemske razrede modulov"
---
> Popolna API dokumentacija za XOOPS sistem modulov.

---

## Arhitektura sistema modulov
```mermaid
graph TB
...
|----------|------|-------------|
| `mid` | int | ID modula |
| `name` | niz | Prikazno ime |
| `version` | niz | Številka različice |
| `dirname` | niz | Ime imenika |
| `isactive` | int | Aktivno stanje (0/1) |
| `hasmain` | int | Ima glavno območje |
| `hasadmin` | int | Ima skrbniško območje |
| `hassearch` | int | Ima funkcijo iskanja |
| `hasconfig` | int | Ima konfiguracijo |
| `hascomments` | int | Ima komentarje |
| `hasnotification` | int | Ima obvestila |
