...
Bloki so vsebinski pripomočki za večkratno uporabo, prikazani v stranskih vrsticah teme in območjih vsebine. Ta priročnik pokriva ustvarjanje, konfiguriranje in prilagajanje blokov XOOPS.

## Struktura blokov

### Definicija bloka v xoops_version.php
```php
$modversion['blocks'][] = [
...
| Parameter | Opis |
|-----------|-------------|
| `file` | PHP datoteka, ki vsebuje blokovne funkcije |
| `name` | Jezikovna konstanta za naslov bloka |
| `description` | Jezikovna konstanta za opis |
| `show_func` | Funkcija za upodabljanje vsebine bloka |
| `edit_func` | Funkcija za upodabljanje obrazca možnosti bloka |
| `template` | Datoteka predloge Smarty |
| `options` | Privzete možnosti, ločene s cevmi |
...
```
## Blokiraj s podporo za kloniranje

Bloki, ki jih je mogoče klonirati, omogočajo več primerkov:
```php
$modversion['blocks'][] = [
...
1. **Cache Results** - Predpomnite drage poizvedbe
2. **Potrdi možnosti** - Vedno preveri možnosti bloka
3. **Escape Output** - Prečisti vso uporabniško vsebino
4. **Uporabi merila** - sestavite poizvedbe z razredom meril
5. **Omeji poizvedbe** - Nastavite razumne omejitve za zmogljivost
6. **Odzivne predloge** - Zagotovite, da bloki delujejo na mobilnih napravah

## Povezana dokumentacija

- Razvoj modula - Vodnik za ustvarjanje modula
- ../02-Core-Concepts/Templates/Smarty-Templating - Sintaksa predloge
