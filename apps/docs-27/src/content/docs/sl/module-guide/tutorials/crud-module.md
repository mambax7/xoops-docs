---
title: "Gradnja modula CRUD"
description: "Popolna vadnica za izdelavo modula CRUD z operacijami baze podatkov, obrazci in skrbniškim vmesnikom"
---
# Izdelava CRUD modula Vadnica

Ta vadnica vas vodi skozi izdelavo celotnega modula CRUD (ustvari, preberi, posodobi, izbriši) za XOOPS. Ustvarili bomo modul »Notes«, ki uporabnikom omogoča upravljanje osebnih zapiskov.

## Predpogoji

- Dokončana vadnica Hello-World-Module
- Razumevanje konceptov PHP OOP
- Osnovno SQL znanje

## Pregled modula

**Funkcije modula Notes:**
- Ustvarite, si oglejte, uredite in izbrišite zapiske
- Skrbniški vmesnik za upravljanje
- Opombe, specifične za uporabnika
- Organizacija kategorij
- Funkcionalnost iskanja

## 1. korak: Struktura imenika

Ustvarite naslednjo strukturo v `/modules/notes/`:
```
/modules/notes/
    /admin/
        admin_header.php
    ...
    xoops_version.php
```
## 2. korak: Shema baze podatkov

Ustvari `sql/mysql.sql`:
```sql
...
```
## 4. korak: Razredi entitet

### Entiteta opombe

Ustvari `class/Note.php`:
```php
<?php
/**
 * Note Entity Class
...
```
### Obravnavalec kategorij

Ustvari `class/CategoryHandler.php`:
```php
<?php
/**
...
```
## 5. korak: Skupna vključena datoteka

Ustvari `include/common.php`:
```php
<?php
/**
 * Common Include File
...
Ustvari `templates/notes_index.tpl`:
```smarty
...
Ustvari `templates/notes_view.tpl`:
```smarty
<{* Note View Template *}>
...
Ustvari `templates/notes_edit.tpl`:
```smarty
<{* Note Edit Template *}>
...
```
### Upravljanje skrbniških zapiskov

Ustvari `admin/notes.php`:
```php
<?php
/**
...
čestitke! Zgradili ste celoten modul CRUD. Zajeti ključni pojmi:

1. **Načrt baze podatkov** - tabele z relacijami
2. **Razredi entitet** - XoopsObject s tipiziranimi lastnostmi
3. **Handler Classes** - XoopsPersistableObjectHandler z metodami po meri
4. **Sprednje strani** - Seznam, ogled in urejanje
5. **Ravnanje z obrazci** - CSRF zaščita in validacija
6. **Administratorski vmesnik** - zasloni za upravljanje
7. **Predloge** - pametne predloge z logiko

## Naslednji koraki

- Dodajte več naprednih funkcij (komentarji, ocene, deljenje)
- Implementirajte ../Patterns/Repository-Pattern za čistejši dostop do podatkov
- Uporabite ../Patterns/MVC-Vzorec za boljšo organizacijo kode
- Dodajte ../Best-Practices/Testing s PHPUnit

Glej tudi: ../Module-Development | ../Patterns/MVC-Pattern | ../Patterns/Repository-Pattern
