---
title: "Beli zaslon smrti (WSOD)"
description: "Diagnosticiranje in odpravljanje belega zaslona smrti v XOOPS"
---
> Kako diagnosticirati in popraviti prazne bele strani v XOOPS.
...
### 1. korak: Omogočite PHP Prikaz napak

Začasno dodaj k `mainfile.php`:
```php
<?php
...
```
### 2. korak: Preverite PHP dnevnik napak
```bash
...
pie title WSOD Common Causes
    ...
    "Timeout" : 5
```
### 1. Omejitev pomnilnika je presežena

**Simptomi:**
- Prazna stran o velikih operacijah
- Deluje za majhne podatke, ne uspe za velike
...
memory_limit = 256M
```
### 2. PHP Sintaksna napaka

**Simptomi:**
- WSOD po urejanju datoteke PHP
- Določena stran ne uspe, druge delujejo
...
```
### 3. Manjka zahtevana datoteka

**Simptomi:**
- WSOD po upload/migration
- Naključne strani ne uspejo
...
ls -la class/
```
### 4. Povezava z bazo podatkov ni uspela

**Simptomi:**
- Vse strani prikazujejo WSOD
- Statične datoteke (slike, CSS) delujejo
...
```
### 5. Težave z dovoljenji

**Simptomi:**
- WSOD pri pisanju datotek
- Cache/compile napak
...
```
### 6. Napaka predloge Smarty

**Simptomi:**
- WSOD na določenih straneh
- Deluje po čiščenju predpomnilnika
...
```
### 7. Najdaljši čas izvedbe

**Simptomi:**
- WSOD po ~30 sekundah
- Dolge operacije ne uspejo
...
Ustvarite `debug.php` v korenu XOOPS:
```php
<?php
/**
 * XOOPS Debug Script
...
```
1. **Vedno naredite varnostno kopijo**, preden naredite spremembe
2. **Preizkusite lokalno** pred uvedbo
3. **Redno spremljajte dnevnike napak**
4. **Uporabite git** za sledenje spremembam
5. **Posodabljajte PHP** znotraj podprtih različic
...
#XOOPS #odpravljanje težav #wsod #debugging #errors