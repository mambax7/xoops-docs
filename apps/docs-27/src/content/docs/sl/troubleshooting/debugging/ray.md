---
title: "Uporaba programa Ray Debugger"
description: "Odpravljanje napak XOOPS aplikacij z razhroščevalnikom Ray"
---
# Uporaba Ray Debuggerja za XOOPS

> Sodobno odpravljanje napak z Ray: preglejte spremenljivke, sporočila dnevnika, sledite SQL poizvedbam in učinkovitosti profila v vaši XOOPS aplikaciji.

---

## Kaj je Ray?

Ray je lahko orodje za odpravljanje napak, ki vam pomaga pregledati stanje aplikacije brez zaustavitve izvajanja ali uporabe prekinitvenih točk. Popoln je za razvoj XOOPS.

**Lastnosti:**
- Dnevnik sporočil in spremenljivk
- Preglejte SQL poizvedbe
- Sledite uspešnosti
- Koda profila
- Dnevniki, povezani s skupino
- Vizualna časovnica

**Zahteve:**
- PHP 7,4+
- Aplikacija Ray (na voljo je brezplačna različica)
- Skladatelj
...
```
### 2. korak: Prenesite aplikacijo Ray

Prenos iz [ray.so](https://ray.so):
- Mac: Ray.app
- Windows: Ray.exe
- Linux: ray (AppImage)

### 3. korak: Konfigurirajte požarni zid (če je potrebno)

Ray privzeto uporablja vrata 23517:
```bash
# UFW
...
?>
```
### 4. Združevanje povezanih dnevnikov
```php
<?php
$ray = ray();
...
## XOOPS-Posebno odpravljanje napak

### Razhroščevanje modula
```php
<?php
...
## Poteki dela za odpravljanje napak

### Odpravljanje napak pri namestitvi modula
```php
<?php
...
## Odpravljanje težav Ray

### V: Ray ne prejema sporočil

**A:**
1. Preverite, ali se aplikacija Ray izvaja
2. Preverite, ali požarni zid dovoljuje vrata 23517
3. Preverite, ali je Ray nameščen:
```bash
composer require spatie/ray
```
### V: Ne vidim SQL poizvedb
...
?>
```
### V: Vpliv Raya na zmogljivost

**A:** Ray ima minimalne stroške. Za produkcijo odstranite klice Ray ali onemogočite:
```php
<?php
