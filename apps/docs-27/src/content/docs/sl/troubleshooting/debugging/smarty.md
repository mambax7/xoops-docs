---
title: "Odpravljanje napak v predlogi Smarty"
description: "Napredne tehnike odpravljanja napak v predlogah in težav za XOOPS"
---
> Napredne tehnike za odpravljanje napak v predlogah Smarty v XOOPS temah in modulih.
...
## Omogoči način Smarty Debug

### 1. način: skrbniška plošča

XOOPS Admin > Settings > Performance:
- Omogoči "Debug Output"
- Nastavite "Debug Level" na 2

---

### 2. način: Konfiguracija kode
```php
<?php
...
### 3. način: pojavno okno za odpravljanje napak v brskalniku
```smarty
{* Add to template to enable debug in footer *}
{debug}
```
To prikaže pojavno okno z vsemi dodeljenimi spremenljivkami.

---

## Pogoste tehnike odpravljanja napak Smarty

### Izpis vseh spremenljivk
```php
<?php
// In PHP code
$tpl = new XoopsTpl();
...
### Specifična spremenljivka dnevnika
```php
<?php
$tpl = new XoopsTpl();
...
### Preverite spremenljivko v predlogi
```smarty
...
## Oglejte si prevedene predloge

Smarty sestavi predloge na PHP za učinkovitost. Odpravljanje napak z ogledom prevedene kode:
```bash
...
## Analizirajte kompilacijo predloge
```php
<?php
...
## Odpravljanje napak, specifičnih za težave

### 1. težava: spremenljivka je prikazana kot prazna
```php
<?php
$tpl = new XoopsTpl();
...
### 2. težava: matričnega ključa ni bilo mogoče najti
```smarty
...
### Težava 3: Plugin/Modifier Ni najdeno
```php
<?php
...
### Težava 4: Prikaz ugnezdenega polja
```smarty
{* Debug nested arrays *}
...
## Ustvari predlogo za odpravljanje napak
```smarty
...
## Odpravljanje napak v delovanju

### Izmeri upodabljanje predloge
```php
<?php
...
?>
```
### Preverite učinkovitost predpomnilnika
```php
<?php
$compile_dir = XOOPS_CACHE_PATH . '/smarty_compile';
...
## Počisti in obnovi predpomnilnik
```php
<?php
...
### Postopek odpravljanja napak po korakih
```mermaid
graph TD
...
## Funkcije za pomoč pri odpravljanju napak
```php
<?php
