---
title: "Skupinski sistem"
description: "Obsežen vodnik za upravljanje skupin XOOPS, privzete skupine, skupine po meri in hierarhijo skupin"
---
# Skupinski sistem v XOOPS

Sistem skupin XOOPS zagotavlja hierarhični okvir za organiziranje uporabnikov in upravljanje kolektivnih dovoljenj. Ta dokument zajema privzete skupine, ustvarjanje skupin po meri, hierarhijo in praktično izvedbo.

## Privzete skupine

XOOPS vključuje tri temeljne skupine, ki se ustvarijo med namestitvijo sistema:

### Skupina spletnih skrbnikov (ID: 1)

Skupina Webmasters predstavlja skrbnike strani s polnim sistemskim dostopom.

**Lastnosti:**
- ID skupine: 1
- Najvišja raven privilegijev
- Ni mogoče izbrisati
- Popoln dostop do vseh modulov in funkcij
...
```
### Skupina registriranih uporabnikov (ID: 2)

Skupina registriranih uporabnikov vključuje vse overjene uporabnike, ki niso anonimni.

**Lastnosti:**
- ID skupine: 2
- Privzeta skupina za nove registracije
- Lahko dostopa do uporabniško specifičnih funkcij
- Odvisno od skupinskih dovoljenj
- Lahko se prilagodi standardnim uporabniškim funkcijam
```php
<?php
/**
...
```
### Anonimna skupina (ID: 3)

Skupina Anonymous predstavlja neavtentificirane obiskovalce spletnega mesta.

**Lastnosti:**
- ID skupine: 3
- Privzeta skupina za neprijavljene uporabnike
- Običajno omejen dostop samo za branje
- Vsebine ni mogoče spreminjati
- Dovoljenja za javni ogled
```php
<?php
/**
...
```
## Ustvarjanje skupin po meri

### Rokovnik za ustvarjanje skupine
```php
<?php
/**
...
```
## Dodelitev skupinskih dovoljenj

### Dodeljevanje dovoljenj skupinam
```php
<?php
/**
...
```
### Preverjanje uporabniških skupin
```php
<?php
/**
...
1. **Jasno poimenovanje**: Uporabite opisna, jasna imena skupin
2. **Dokumentacija**: Nameni in dovoljenja skupine dokumentov
3. **Načelo najmanjših privilegijev**: Dodelite minimalna zahtevana dovoljenja
4. **Redne revizije**: redno pregledujte članstvo v skupini in dovoljenja
5. **Privzete skupine**: Ohranite privzete skupine (spletni skrbniki, registrirani, anonimni)
