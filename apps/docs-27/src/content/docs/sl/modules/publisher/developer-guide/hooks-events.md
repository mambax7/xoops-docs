---
title: "Založnik - Trnki in dogodki"
description: "Vodnik za razširitev Publisherja z uporabo kavljev, dogodkov in sistema vtičnikov"
---
# Trnki in dogodki založnika

> Popoln vodnik za razširitev funkcionalnosti Publisherja z uporabo dogodkov, kavljev in vtičnikov.

---

## Pregled sistema dogodkov

### Kaj so dogodki?

Dogodki omogočajo drugim modulom, da se odzovejo na dejanja izdajatelja:
```
...
### Dogodki predmeta (članka).

#### publisher.item.created

Sproži se, ko je ustvarjen nov članek.
```php
...
Sproži se ob posodobitvi članka.
```php
...
Izpuščeno, ko je članek izbrisan.
```php
...
Sproži se, ko se status članka spremeni v objavljen.
```php
...
Odpuščeno, ko je članek v teku odobren.
```php
...
Odpuščeno, ko je članek zavrnjen.
```php
...
Sproži se, ko je kategorija ustvarjena.
```php
...
Sproži se ob posodobitvi kategorije.
```php
...
Sproži se, ko je kategorija izbrisana.
```php
...
Sprožen, ko je komentar objavljen.
```php
...
Sprožen, ko je komentar odobren.
```php
...
### Primer 1: Pošljite e-pošto o ustvarjanju članka
```php
<?php
// Listen for article creation
xoops_events()->attach(
...
?>
```
### Primer 2: Posodobite indeks iskanja
```php
<?php
...
?>
```
### Primer 3: Samodejna objava v družabnih medijih
```php
<?php
...
?>
```
### Primer 4: Sinhronizacija z zunanjim sistemom
```php
<?php
...
Kavlji omogočajo spremembe vedenja založnika:

#### publisher.view.article.start

Poklicano, preden je članek upodobljen.
```php
xoops_events()->attach(
    'publisher.view.article.start',
...
Poklicano po upodobitvi članka.
```php
xoops_events()->attach(
    'publisher.view.article.end',
    'append_to_article'
...
Poklicano pri preverjanju dovoljenj.
```php
xoops_events()->attach(
    'publisher.permission.check',
...
Vtičniki razširjajo funkcionalnost založnika:
...
Filtri spreminjajo podatke before/after obdelava:
```php
<?php
...
Izvedite kodo na določenih točkah:
```php
<?php
// Do action
...
## Razširitev z vtičniki

### Primer vtičnika: Sorodni članki
```php
<?php
...
### Smernice za poslušalce dogodkov
```php
...
- [Založnik GitHub](https://github.com/XoopsModules25x/publisher)
- [XOOPS Sistem dogodkov](../../03-Module-Development/Module-Development.md)
- [Razvoj vtičnika](../../03-Module-Development/Module-Development.md)
