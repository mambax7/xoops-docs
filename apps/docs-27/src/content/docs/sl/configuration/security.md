---
title: "Varnostna konfiguracija"
description: "Popoln vodnik za utrjevanje varnosti za XOOPS, vključno z dovoljenji za datoteke, SSL/HTTPS, občutljivimi imeniki in najboljšimi varnostnimi praksami"
---
# XOOPS Varnostna konfiguracija

Obsežen vodnik za zaščito vaše namestitve XOOPS pred pogostimi spletnimi ranljivostmi.

## Varnostni kontrolni seznam

Preden zaženete svoje spletno mesto, uvedite te varnostne ukrepe:
...
Ustrezna dovoljenja za datoteke so ključnega pomena za varnost.

#### Smernice za dovoljenja

| Pot | Dovoljenja | Lastnik | Razlog |
|---|---|---|---|
...
| konfiguracije/ | 755 | koren | Berljivo, ne zapisljivo |#### Nastavitev skripta za dovoljenja
```bash
#!/bin/bash
...
**CRITICAL:** Namestitveno mapo morate po namestitvi odstraniti!
```bash
...
```
### Zaščitite občutljive imenike

Ustvarite datoteke .htaccess za blokiranje spletnega dostopa do občutljivih map:
...
```
### Zaščitite imenik za nalaganje

Prepreči izvajanje skriptov pri nalaganjih:
...
Šifrirajte ves promet med uporabniki in vašim strežnikom.

### Pridobite SSL certifikat

**1. možnost: brezplačno potrdilo podjetja Let's Encrypt**
```bash
# Install Certbot
...
```
**2. možnost: komercialno potrdilo SSL**

Kontaktirajte SSL ponudnika ali registrarja:
1. Nakup potrdila SSL
2. Preverite lastništvo domene
3. Namestite datoteke potrdila na strežnik
4. Konfigurirajte spletni strežnik

### Apache SSL Konfiguracija

Ustvarite HTTPS virtualnega gostitelja:
...
```
### Samodejna obnovitev potrdila Let's Encrypt
```bash
# Enable auto-renewal
...
XOOPS uporablja parametrizirane poizvedbe (privzeto varne), vendar vedno:
```php
...
```
### Skriptno izvajanje med spletnimi mesti (XSS) Preprečevanje

Vedno razčisti uporabniški vnos:
```php
// UNSAFE
...
```
### Ponarejanje zahtev med spletnimi mesti (CSRF) Preprečevanje

XOOPS vključuje zaščito žetonov CSRF. Vedno vključi žetone:
```html
<!-- In forms -->
<form method="post">
...
```
### Onemogoči PHP izvajanje v mapi za nalaganje

Preprečite napadalcem nalaganje in izvajanje PHP:
```bash
...
Konfigurirajte pomembne varnostne glave HTTP:
```apache
...
```
## Varnost skrbniške plošče

### Preimenuj skrbniško mapo

Zaščitite skrbniško mapo tako, da jo preimenujete:
```bash
# Rename admin folder
...
```
Konfigurirajte XOOPS za uporabo preimenovane mape:
...
```
### Seznam dovoljenih naslovov IP za skrbnika

Omejite skrbniški dostop na določene IP-je:
...
```
### Močne skrbniške poverilnice

Uveljavi močna gesla za skrbnike:

1. Uporabite vsaj 16 znakov
2. Mešajte velike in male črke, številke, simbole
3. Redno menjajte geslo (vsakih 90 dni)
4. Uporabite upravitelja gesel
5. Omogočite dvostopenjsko avtentikacijo, če je na voljo

### Spremljajte dejavnost skrbnika

Omogoči beleženje skrbniške prijave:
...
### Posodobite XOOPS in module

Posodabljajte XOOPS in vse module:
```bash
...
Avtomatizirajte dnevne varnostne kopije:
```bash
#!/bin/bash
# Daily backup script
...
```
## Predloga varnostnega kontrolnega seznama

Uporabite to predlogo za redne varnostne revizije:
```
