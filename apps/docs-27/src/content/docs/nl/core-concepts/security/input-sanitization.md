---
title: "Invoersanering"
description: "Gebruik van MyTextSanitizer en validatietechnieken in XOOPS"
---
Vertrouw nooit op gebruikersinvoer. Valideer en zuiver altijd alle invoergegevens voordat u deze gebruikt. XOOPS biedt de klasse `MyTextSanitizer` voor het opschonen van tekstinvoer en verschillende helperfuncties voor validatie.
...
**Vertrouw nooit gebruikersinvoer.** Alle gegevens uit externe bronnen moeten:

1. **Gevalideerd**: controleer of het overeenkomt met het verwachte formaat en type
2. **Opgeschoond**: verwijder of ontsnap aan potentieel gevaarlijke karakters
3. **Escaped**: bij uitvoer ontsnappen voor de specifieke context (HTML, JavaScript, SQL)

## MyTextSanitizer-klasse

XOOPS biedt de klasse `MyTextSanitizer` (gewoonlijk alias `$myts`) voor het opschonen van tekst.

### Het exemplaar ophalen
...
### Tekstgebiedinhoudverwerking

De `displayTarea()`-methode biedt uitgebreide tekstgebiedverwerking:
...
## Verschillende invoertypen verwerken
...
Bij gebruik van XMF zorgt de klasse Request voor een schonere invoerverwerking:
...
## Een validatieklasse creëren

Voor complexe formulieren maakt u een speciale validatieklasse:
...
## Opschonen voor databaseopslag

Bij het opslaan van gegevens in de database:
...
## Ontsmetting voor weergave

Verschillende contexten vereisen verschillende ontsnappingen:
...
**Probleem**: gegevens worden meerdere keren gecodeerd
...
### Inconsistente codering

**Probleem**: Sommige uitgangen zijn gecodeerd, andere niet

**Oplossing**: gebruik altijd een consistente aanpak, bij voorkeur codering bij uitvoer:
...
**Probleem**: Alleen ontsmetten zonder valideren

**Oplossing**: Altijd eerst valideren en dan ontsmetten:
...
1. **Gebruik MyTextSanitizer** voor de verwerking van tekstinhoud
2. **Gebruik filter_var()** voor specifieke formaatvalidatie
3. **Gebruik type casting** voor numerieke waarden
4. **Toegestane waarden op witte lijst** voor geselecteerde invoer
5. **Valideren vóór ontsmetten**
6. **Escape op uitgang**, niet op ingang
7. **Gebruik voorbereide instructies** voor databasequery's
8. **Maak validatieklassen** voor complexe formulieren
9. **Vertrouw nooit op validatie aan de clientzijde** - valideer altijd aan de serverzijde

---

#security #sanitization #validatie #xoops #MyTextSanitizer #input-handling