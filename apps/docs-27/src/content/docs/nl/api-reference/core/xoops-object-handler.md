...
description: "Basishandlerklasse voor CRUD-bewerkingen op XoopsObject-instanties met databasepersistentie"
---
...
Creëert een nieuwe objectinstantie.
...
Haalt een object op via zijn primaire sleutel.
...
Slaat een object op in de database (invoegen of bijwerken).
...
De `XoopsPersistableObjectHandler` breidt de `XoopsObjectHandler` uit met extra methoden voor query's en bulkbewerkingen.
...
Haalt meerdere objecten op die aan de criteria voldoen.
...
Telt objecten die voldoen aan de criteria.
...
Haalt alle objecten op (alias voor getObjects zonder criteria).
...
Haalt alleen de primaire sleutels van overeenkomende objecten op.
...
Haalt een sleutel-waardelijst op voor vervolgkeuzelijsten.
```php
public function getList(CriteriaElement $criteria = null): array
```

**Retourneert:** `array` - Associatieve array [id => identifier]
...
Verwijdert alle objecten die aan de criteria voldoen.
...
Werkt een veldwaarde bij voor alle overeenkomende objecten.
...
De uitgebreide insteekmethode met extra functionaliteit.
...
**Gedrag:**
- Als het object nieuw is (`isNew() === true`): INSERT
- Als object bestaat (`isNew() === false`): UPDATE
- Roept automatisch `cleanVars()` op
- Stelt de ID voor automatisch verhogen in voor nieuwe objecten
...
Globale functie voor het ophalen van een kernhandler.
...
Haalt een modulespecifieke handler op.
...
## Aangepaste handlers maken

### Basishandlerimplementatie
...
### De aangepaste afhandeling gebruiken
...
1. **Gebruik criteria voor query's**: gebruik altijd Criteria-objecten voor typeveilige query's

2. **Uitbreiden voor aangepaste methoden**: voeg domeinspecifieke querymethoden toe aan handlers

3. **Invoegen/verwijderen overschrijven**: voeg cascadebewerkingen en tijdstempels toe aan overschrijvingen

4. **Gebruik transacties waar nodig**: verpak complexe operaties in transacties

5. **Maak gebruik van getList**: gebruik `getList()` voor geselecteerde vervolgkeuzelijsten om het aantal zoekopdrachten te verminderen

6. **Indexsleutels**: Zorg ervoor dat databasevelden die in criteria worden gebruikt, worden geïndexeerd

7. **Beperk resultaten**: gebruik altijd `setLimit()` voor potentieel grote resultatensets

## Gerelateerde documentatie

- XoopsObject - Basisobjectklasse
