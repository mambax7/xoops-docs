---
title: "xoops_version.php - Modulemanifest"
---
## Overzicht

Het `xoops_version.php`-bestand vormt het hart van elke XOOPS-module. Het definieert modulemetagegevens, databasetabellen, sjablonen, blokken, configuratieopties en installatiehaken.
...
|-----|------|------------|
| `name` | tekenreeks | Weergavenaam (gebruik taalconstante) |
| `version` | tekenreeks | Semantische versie (MAJOR.MINOR.PATCH) |
| `description` | tekenreeks | Modulebeschrijving |
| `author` | tekenreeks | Naam primaire auteur |
| `credits` | tekenreeks | Extra bijdragers |
| `license` | tekenreeks | Licentienaam |
...
| formuliertype | waardetype | Beschrijving |
|----------|-----------|------------|
| `textbox` | `text`/`int` | Enkellijnige invoer |
| `textarea` | `text` | Meerlijnige invoer |
| `yesno` | `int` | Ja/Nee radio |
| `select` | `text` | Vervolgkeuzelijst selecteer |
| `select_multi` | `array` | Meervoudig selecteren |
| `group` | `int` | Groepkiezer |
| `group_multi` | `array` | Keuzeschakelaar voor meerdere groepen |
| `user` | `int` | Gebruikersselector |
| `color` | `text` | Kleurkiezer |
| `hidden` | `text` | Verborgen veld |
...
- Module-ontwikkeling - Volledige modulegids
- Modulestructuur - Directorystructuur
- Blokontwikkeling - Blokken maken
- Databasebewerkingen - Database-instellingen