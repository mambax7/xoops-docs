---
title: "Hệ thống cấp phép"
description: "Hướng dẫn đầy đủ về hệ thống cấp phép XOOPS, các loại, kiểm tra, phân cấp và kiểm soát truy cập"
---
# Hệ thống cấp phép trong XOOPS

Hệ thống cấp phép XOOPS là một khung kiểm soát truy cập chi tiết giúp quản lý ai có thể thực hiện hành động nào trên tài nguyên nào. Tài liệu này bao gồm các loại quyền, cơ chế kiểm tra, phân cấp và ví dụ triển khai.

## Các loại quyền

### Quyền cấp mô-đun

Quyền cấp mô-đun kiểm soát quyền truy cập vào toàn bộ modules hoặc các chức năng mô-đun.

**Tên giấy phép phổ biến:**
- `module_view` - Xem nội dung mô-đun
- `module_read` - Đọc tài nguyên mô-đun
- `module_submit` - Gửi nội dung đến mô-đun
- `module_edit` - Chỉnh sửa nội dung mô-đun
- `module_admin` - Quản trị module
...
Quyền cấp mục kiểm soát quyền truy cập vào các tài nguyên cụ thể trong một mô-đun.

**Ví dụ:**
- ID bài viết: Nhóm có thể xem/chỉnh sửa bài viết cụ thể không?
- ID danh mục: Nhóm có thể truy cập danh mục không?
- ID trang: Nhóm có thể xem/sửa đổi trang cụ thể không?
...
Quyền chặn kiểm soát khả năng hiển thị và tương tác với các khối được hiển thị trên các trang.
...
Quyền kiểm soát quản lý nhóm và administration.
...
`XoopsGroupPermHandler` class cung cấp các phương pháp kiểm tra và quản lý quyền.
...
### Kiểm tra quyền của bảng quản trị
...
### Kiểm tra quyền được tối ưu hóa
...
1. **Nguyên tắc đặc quyền tối thiểu**: Chỉ cấp các quyền cần thiết
2. **Quyền truy cập dựa trên vai trò**: Sử dụng các nhóm để có quyền dựa trên vai trò
3. **Kiểm tra thường xuyên**: Định kỳ xem xét các quyền
4. **Tách nhiệm vụ**: Tách admin khỏi quyền của người dùng
5. **Từ chối rõ ràng**: Từ chối mặc định, cách tiếp cận cho phép rõ ràng
...
#quyền #kiểm soát truy cập #bảo mật #ủy quyền #acl #kiểm tra quyền